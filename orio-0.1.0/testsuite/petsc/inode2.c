...
PetscErrorCode MatAssemblyEnd_Inode(Mat A, MatAssemblyType mode) {
    ...
    ierr = Mat_CheckInode(A,samestructure);
    CHKERRQ(ierr);
...
PetscErrorCode MatDestroy_Inode(Mat A) {
    ...
    PetscFunctionBegin;
    ierr = PetscFree(a->inode.size);
    CHKERRQ(ierr);
    ierr = PetscFree2(a->inode.ibdiag,a->inode.bdiag);
    CHKERRQ(ierr);
    ierr = PetscObjectComposeFunctionDynamic((PetscObject)A,"MatInodeAdjustForInodes_C","",PETSC_NULL);
    CHKERRQ(ierr);
    ierr = PetscObjectComposeFunctionDynamic((PetscObject)A,"MatInodeGetInodeSizes_C","",PETSC_NULL);
    CHKERRQ(ierr);
    PetscFunctionReturn(0);
}
...
PetscErrorCode MatCreate_Inode(Mat B) {
    ...
    ierr = PetscOptionsBegin(((PetscObject)B)->comm,((PetscObject)B)->prefix,"Options for SEQAIJ matrix","Mat");
    CHKERRQ(ierr);
    ierr = PetscOptionsTruth("-mat_no_unroll","Do not optimize for inodes (slower)",PETSC_NULL,no_unroll,&no_unroll,PETSC_NULL);
    CHKERRQ(ierr);
    if (no_unroll) {
        ierr = PetscInfo(B,"Not using Inode routines due to -mat_no_unroll\n");
        CHKERRQ(ierr);
    }
    ierr = PetscOptionsTruth("-mat_no_inode","Do not optimize for inodes (slower)",PETSC_NULL,no_inode,&no_inode,PETSC_NULL);
    CHKERRQ(ierr);
    if (no_inode) {
        ierr = PetscInfo(B,"Not using Inode routines due to -mat_no_inode\n");
        CHKERRQ(ierr);
    }
    ierr = PetscOptionsInt("-mat_inode_limit","Do not use inodes larger then this value",PETSC_NULL,b->inode.limit,&b->inode.limit,PETSC_NULL);
    CHKERRQ(ierr);
...
PetscErrorCode MatDuplicate_Inode(Mat A,MatDuplicateOption cpvalues,Mat *C) {
    ...
    if (a->inode.size) {
        ierr                = PetscMalloc((m+1)*sizeof(PetscInt),&c->inode.size);
        CHKERRQ(ierr);
        c->inode.node_count = a->inode.node_count;
        ierr                = PetscMemcpy(c->inode.size,a->inode.size,(m+1)*sizeof(PetscInt));
        CHKERRQ(ierr);
    } else {
        c->inode.size       = 0;
...
PetscErrorCode MatILUDTFactor_Inode(Mat A,IS isrow,IS iscol,MatFactorInfo *info,Mat *fact) {
    ...
    PetscFunctionBegin;
    /* check for identical nodes. If found, use inode functions */
    ierr = Mat_CheckInode(*fact,PETSC_FALSE);
    CHKERRQ(ierr);
    PetscFunctionReturn(0);
}
