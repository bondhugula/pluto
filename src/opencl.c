 ...
 *
    opencl_codegen_generic_code(); // this function will generate the code that remains unchanged across programs. (like CreateContext etc)
    opencl_codegen_macros(); // generate the macros that conveys the size of our variables
    opencl_codegen_variables(); // generate the variables to be used by the CPU version of the opencl code
    opencl_codegen_headerfile(); // generates the header file and its entries specific to this program
    opencl_codegen_createbuffer(); // generate the createBuffers function
    opencl_codegen_runcomppackkernel(); // generate the runComputePackKernel code
    opencl_codegen_initdatadistrib(); // the InitialDataDistribution code
    opencl_codegen_setinitkernelargs();
    opencl_codegen_copyflowoutdata(); // the code for copying out data from their originating devices 
    opencl_codegen_copyflowindata(); // the code for copying in the data to their receiving devices
    opencl_codegen_computeglobaloffsets(); // computing the kernel offsets to be used in each device
    opencl_codegen_enqueuecomputekernels(); // the GPU version of the runComputePackKernel
    opencl_codegen_cleanupresources(); // CleanupResources. Does the opposite of what CreateBuffers does
    ...
    opencl_codegen_computekernel(); // the pluto generated tiled and paralleled code for CPU computations
*/
...
{
   // we have to generate the following functions. for now we assume the opencl kernel is given to us.
