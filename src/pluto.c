    ...
    do{
        ...
        if (bestsol != NULL)    {
            ...
            if (options->varliberalize) {
                for (j=0; j<ndeps; j++) {
                    if (deps[j]->temp_across && num_sols_found < deps[j]->fuse_depth 
                            && pluto_domain_equality(stmts[prog->deps[j]->src], stmts[prog->deps[j]->dest])) {
                        for (k=0; k<nvar; k++) {
...
int pluto_auto_transform(PlutoProg *prog)
{
    ...
    if (options->varliberalize) {
        pluto_variable_liberalize(prog);
    }
