...
char *pluto_dist_gen_declarations(char *arr_name, PlutoProg *prog) {
  ...
  PlutoConstraints *data_tiles =
      pluto_dist_get_required_data_tiles(arr->array_bounds, 0, arr_name, prog);
  ...
  for (i = 0; i < arr->num_tiled_loops; ++i) {
    sprintf(index + strlen(index), "[%s_size_%d]", arr->text,
            arr->first_tile_dim + i);

    sprintf(size_text + strlen(size_text), "%s_size_%d", arr->text,
            arr->first_tile_dim + i);
  ...
  sprintf(decl + strlen(decl), "%s_trans = (%s **)malloc((%s) * sizeof(%s));\n",
          arr->text, arr->data_type, size_text, arr->data_type);
...
void pluto_dist_update_arr_domain(struct stmt_access_pair **acc_stmts,
  ...
  for (i = 0; i < num_accs; i++) {
    ...
    PlutoConstraints *data_in_one =
        pluto_compute_region_data(stmt, srcdomain, acc, src_copy_level, prog);

    if (all_data == NULL)
      all_data = pluto_constraints_dup(data_in_one);
    else
      all_data = pluto_constraints_unionize(all_data, data_in_one);
...
char *get_data_tile_ref_count_stmt_text(Array *arr, PlutoProg *prog) {
  ...
  sprintf(stmt_text + strlen(stmt_text), "decrement_ref_count(buff_mang_%s, ",
          arr->text);
 ...
 * copy_level[loop_num]: number of outer loops of source loop to be treated as
 * parameters
...
Stmt **gen_data_tile_ref_count_update_code(struct stmt_access_pair **acc_stmts,
   ...
   */
  PlutoConstraints *data_tiles = pluto_dist_get_required_data_tiles(
      arr->parmetric_domain[loop_num], src_copy_level, acc_name, prog);
  ...
  Stmt *alloc_stmt = create_helper_stmt(anchor_stmt, src_copy_level, stmt_text,
                                        DATA_DIST_MANG, loop_num);

  /* Add dimensions that actually scan the data tile space */
  for (j = arr->last_tile_dim; j >= arr->first_tile_dim; --j) {
    pluto_stmt_add_dim(alloc_stmt, alloc_stmt->dim, alloc_stmt->trans->nrows,
                       arr->iterators[j], H_LOOP, prog);
  }
...
char *get_tile_pi_check_ref_count_stmt_text(Array *arr, int src_copy_level,
  ...
  sprintf(stmt_text + strlen(stmt_text), "data_tile_ref_count_init_%s_%d(",
          arr->text, loop_num);
  for (i = 0; i < src_copy_level; i++) {
    sprintf(stmt_text + strlen(stmt_text), "t%d", i + 1);
...
Stmt **gen_write_out_tiles_ref_count_code(struct stmt_access_pair **acc_stmts,
  ...
  char *stmt_text = get_tile_pi_check_ref_count_stmt_text(arr, src_copy_level,
                                                          loop_num, prog);

  Stmt *alloc_stmt = create_helper_stmt(anchor_stmt, src_copy_level, stmt_text,
                                        DATA_DIST_INIT, loop_num);
...
char *get_data_tile_ref_count_init_stmt_text(Array *arr, int src_copy_level,
  ...
  sprintf(stmt_text + strlen(stmt_text), "increment_ref_count(buff_mang_%s, ",
          arr->text);
...
Stmt **gen_data_tile_alloc_code(struct stmt_access_pair **acc_stmts,
  ...
  Stmt *alloc_stmt = create_helper_stmt(
      anchor_stmt, src_copy_level, alloc_stmt_text, DATA_DIST_MANG, loop_num);
...
Stmt *gen_comm_data_alloc_code(Stmt *anchor_stmt, PlutoConstraints *constraints,
  ...
  PlutoConstraints *data_tiles = pluto_dist_get_required_data_tiles(
      constraints, src_copy_level, acc_name, prog);
...
char *pluto_dist_copy_back_func_call_text(int loop_num, int src_copy_level,
  ...
  sprintf(text + strlen(text), "data_tile_copy_back_%s_%d(", arr->text,
          loop_num);
  for (i = 0; i < src_copy_level; i++) {
    sprintf(text + strlen(text), "t%d", i + 1);
    if (i != src_copy_level - 1)
...
Stmt **gen_copy_back_code(struct stmt_access_pair **racc_stmts, int num_accs,
  ...
  char *copy_back_stmt_text =
      pluto_dist_copy_back_func_call_text(loop_num, src_copy_level, arr, prog);

  Stmt *copy_back_stmt =
      create_helper_stmt(anchor_stmt, src_copy_level, copy_back_stmt_text,
...
Stmt **gen_null_init_code(Array *arr, PlutoProg *prog, int loop_num) {
  ...
  Stmt *ptr_init_stmt = create_helper_stmt(
      prog->stmts[0], 0, ptr_init_stmt_text, DATA_SETUP, loop_num);

  /* Add dimensions that actually scan the data tile space */
  for (j = arr->last_tile_dim; j >= arr->first_tile_dim; --j) {
    pluto_stmt_add_dim(ptr_init_stmt, ptr_init_stmt->dim,
                       ptr_init_stmt->trans->nrows, arr->iterators[j], H_LOOP,
                       prog);
  }
...
char *get_tile_pi_check_read_in_text(Array *arr, int src_copy_level,
  ...
  sprintf(stmt_text + strlen(stmt_text), "read_in_data_tile_alloc_%s_%d(",
          arr->text, loop_num);
  for (i = 0; i < src_copy_level; i++) {
    sprintf(stmt_text + strlen(stmt_text), "t%d", i + 1);
    if (i != src_copy_level - 1)
  ...
  sprintf(stmt_text + strlen(stmt_text), "read_in_copy_%s_%d(", arr->text,
          loop_num);
...
Stmt **gen_read_in_code(PlutoConstraints *read_in,
   ...
   */
  PlutoConstraints *data_tiles = pluto_dist_get_required_data_tiles(
      read_in, src_copy_level, acc_name, prog);
  ...
  char *stmt_text =
      get_tile_pi_check_read_in_text(arr, src_copy_level, loop_num, prog);
...
Stmt **gen_read_in_code_old(struct stmt_access_pair **racc_stmts, int num_accs,
  ...
  for (i = 0; i < num_accs; i++) {
    ...
    if (read_in == NULL)
      read_in = pluto_constraints_dup(read_in_one);
    else
      read_in = pluto_constraints_unionize(read_in, read_in_one);
   ...
   */
  PlutoConstraints *read_in_tiles = pluto_dist_get_required_data_tiles(
      read_in, src_copy_level, acc_name, prog);
  ...
  sprintf(init_stmt_text + strlen(init_stmt_text),
          " __ifndef USE_LOCAL_ARRAYS");
  sprintf(init_stmt_text + strlen(init_stmt_text), " %s ",
          pluto_dist_copy_init_text(arr, prog));
  ...
  Stmt *init_stmt = create_helper_stmt(
      anchor_stmt, src_copy_level, init_stmt_text, DATA_DIST_COPY, loop_num);

  Stmt *alloc_stmt = create_helper_stmt(
      anchor_stmt, src_copy_level, alloc_stmt_text, DATA_DIST_INIT, loop_num);
...
Stmt **gen_write_out_code(struct stmt_access_pair **wacc_stmts, int num_accs,
  ...
  IF_DEBUG(
      printf("Last writer buffer size for %s: %s\n", acc_name, lw_buf_size););
  ...
  sprintf(lw_holder_text + strlen(lw_holder_text),
          "if (pi_%d(%s%s,nprocs) == my_rank)", loop_num, args, params);
  sprintf(lw_holder_text + strlen(lw_holder_text),
          "%s = writeout_pack_%s_%d(%s,%s,%s", lw_count_name, acc_name,
          loop_num, args, lwbufname, lw_count_name);
  if (options->variables_not_global) {
  ...
  char *lw_stmt_text =
      malloc(strlen(lwbufname) + strlen("[") + strlen(lw_count_name) +
             strlen("++] = ") + strlen(access) + 1);
  sprintf(lw_stmt_text, "%s[%s++] = %s", lwbufname, lw_count_name, access);

  fprintf(packfp, "int writeout_pack_%s_%d(%s,double *%s,int %s", acc_name,
          loop_num, decl_args, lwbufname, lw_count_name);
  fprintf(headerfp, "int writeout_pack_%s_%d(%s,double *%s,int %s", acc_name,
          loop_num, decl_args, lwbufname, lw_count_name);
  if (options->variables_not_global) {
  ...
  fprintf(headerfp, ");\n");
  generate_pack_or_unpack(packfp, prog, write_out, lw_stmt_text, IN_FUNCTION,
                          iters, src_copy_level, acc_nrows, acc_name,
                          lw_count_name);
  ...
  Stmt *lw_copy_stmt = create_helper_stmt(anchor_stmt, src_copy_level,
                                          lw_stmt_text, LW_COPY_OUT, loop_num);
  Stmt *lw_holder = create_helper_stmt(anchor_stmt, src_copy_level,
                                       lw_holder_text, LW_COPY_OUT, loop_num);
  ...
  /* Create statement stub to get hold of the outer loops for the copy
   * stmt; rest of the loops to be added are the actual copy loops */
  Stmt *comm_stmt_w = create_helper_stmt(anchor_stmt, outer_dist_loop_level,
                                         comm_text_w, LW_COMM_CALL, loop_num);

  sprintf(prog->decls + strlen(prog->decls),
          "%s = (double *) polyrt_max_alloc(%s, nprocs*lw_buf_size_%s, "
          "&lw_recv_buf_size_%s);\n",
  ...
  for (i = 0; i < src_copy_level + prog->npar; i++) {
    ...
    if (i <= src_copy_level - 1)
      sprintf(lw_proc_stmt_text + strlen(lw_proc_stmt_text), "t%d", i + 1);
    else
      sprintf(lw_proc_stmt_text + strlen(lw_proc_stmt_text), "%s",
              prog->params[i - src_copy_level]);
  }
  ...
  sprintf(lw_proc_stmt_text + strlen(lw_proc_stmt_text),
          ...
          lw_displsname, acc_name, loop_num, args, lwrecvbufname,
          lw_displsname);
  if (options->variables_not_global) {
  ...
  char *lw_copyback_text =
      malloc(strlen(access) + strlen(" = lw_recv_buf_") + strlen(acc_name) +
             strlen("[") + strlen(lw_displsname) + strlen("[proc]+ count++];") +
             1 + strlen(" __ifndef USE_LOCAL_ARRAYS   __endif"));
  sprintf(lw_copyback_text, " __ifndef USE_LOCAL_ARRAYS %s = %s[%s++]; __endif",
          access, lwrecvbufname, lw_displsname);

  Stmt *lw_copyback_stmt = create_helper_stmt(
      anchor_stmt, src_copy_level, lw_copyback_text, LW_COPY_IN, loop_num);
  Stmt *lw_copyback_proc_stmt = create_helper_stmt(
      anchor_stmt, src_copy_level, lw_proc_stmt_text, LW_COPY_IN, loop_num);
  ...
  fprintf(packfp, "int writeout_unpack_%s_%d(%s,double *%s,int %s", acc_name,
          loop_num, decl_args, lwrecvbufname, lw_displsname);
  fprintf(headerfp, "int writeout_unpack_%s_%d(%s,double *%s,int %s", acc_name,
          loop_num, decl_args, lwrecvbufname, lw_displsname);
  if (options->variables_not_global) {
    fprintf(packfp, ", __DECLARATION_OF_%s", acc_name);
    fprintf(headerfp, ", __DECLARATION_OF_%s", acc_name);
  ...
  add_data_dist_parm_decl(headerfp, acc_name, prog);
  fprintf(headerfp, ");\n");
  generate_pack_or_unpack(packfp, prog, write_out, lw_copyback_text, LW_COPY_IN,
                          iters, src_copy_level, acc_nrows, acc_name,
                          lw_displsname);
  if (options->data_dist) {
    //		Array *arr = pluto_get_corrs_array(acc_name, prog);
...
//    Stmt *lw_copyback_proc_stmt = create_helper_stmt(anchor_stmt,
// src_copy_level, lw_proc_stmt_text, LW_COPY_IN, loop_num);
...
void print_data_dist_parm_call(char *str, Array *arr) {
  ...
  if (options->data_dist && arr != NULL) {
    ...
    for (s = 1; s < arr->num_tiled_loops; ++s) {
      sprintf(str + strlen(str), ", %s_size_%d", arr->text,
              s + arr->first_tile_dim);
    }
...
void generate_pack_or_unpack(FILE *packfp, PlutoProg *prog,
  ...
  if (options->data_dist && stmttype != LW_COPY_IN) {
    ...
    pluto_add_stmt(pack, data_tiles, packtrans, new_iter, alloc_stmt_text,
                   stmttype);
    //
...
Stmt *gen_comm_code_count_recvs(struct stmt_access_pair ***wacc_stmts,
  ...
  sprintf(count_recvs_stmt_text, "proc = pi_%d(%s%s, nprocs); ", loop_num, args,
          params);
  ...
  Stmt *count_recvs_stmt = create_helper_stmt(
      anchor_stmt, src_copy_level, count_recvs_stmt_text, COPY_IN, loop_num);
...
Stmt *gen_comm_code_async_recv(struct stmt_access_pair ***wacc_stmts,
  ...
  for (i = 0; i < num_data; i++) {
    ...
    sprintf(async_recv_stmt_text + strlen(async_recv_stmt_text),
            "assert(\"increase __MAX_NUM_RECVS\" && (_num_recvs_%s <= "
            "__MAX_NUM_RECVS));",
            acc_name);
    sprintf(async_recv_stmt_text + strlen(async_recv_stmt_text),
  ...
  Stmt *async_recv_stmt =
      create_helper_stmt(anchor_stmt, src_copy_level - 1, async_recv_stmt_text,
                         COMM_CALL, loop_num);
...
Stmt **gen_tasks_code(Stmt **loop_stmts, int nstmts, int *copy_level,
  ...
  generate_outgoing(loop_stmts, nstmts, copy_level, prog, loop_num, pi_mappings,
                    tasks_loops_decl, outfp, headerfp);
  generate_incoming(loop_stmts, nstmts, copy_level, prog, loop_num, pi_mappings,
                    outfp, headerfp);
  ...
  char *lbexpr;
  char *dim_sizes =
      malloc((strlen("max_num_tasks_loop_dim*") + 10) * src_copy_level + 1);
  strcpy(dim_sizes, "");
  int num_dims = 0;
  ...
  for (i = 0; i < src_copy_level; i++) {
    if (pluto_is_hyperplane_loop(anchor_stmt, i)) {
      ...
      sprintf(prog->decls + strlen(prog->decls),
              "max_num_tasks_loop%d_dim%d = ceil(%s);\n", loop_num, num_dims,
              extent);
      sprintf(prog->decls + strlen(prog->decls),
              "lb_tasks_loop%d_dim%d = floor(%s);\n", loop_num, num_dims,
              lbexpr);
      sprintf(dim_sizes + strlen(dim_sizes), "max_num_tasks_loop%d_dim%d*",
              loop_num, num_dims);
      free(extent);
      free(lbexpr);
      dims[num_dims++] = i;
  ...
  if (options->dynschedule_graph) {
    sprintf(
        prog->decls + strlen(prog->decls),
        "tbb::flow::continue_node<tbb::flow::continue_msg> **tasks_loop%d;\n",
        loop_num);
    sprintf(prog->decls + strlen(prog->decls),
            "tasks_loop%d = (tbb::flow::continue_node<tbb::flow::continue_msg> "
            "**)malloc(%ssizeof(tbb::flow::continue_node<tbb::flow::continue_"
            "msg> *));\n",
            loop_num, dim_sizes);
  } else {
    ...
    sprintf(prog->decls + strlen(prog->decls),
            "tasks_loop%d = (int *)malloc(%ssizeof(int));\n", loop_num,
            dim_sizes);
  }
  free(dim_sizes);
  pluto_constraints_free(anchor_stmt_new_dom);
  ...
  if (options->distmem) {
    ...
    // owned tasks
    sprintf(init_all_tasks_text + strlen(init_all_tasks_text),
            "_num_tasks_to_execute++;");
    // initialize firing count of owned tasks
    sprintf(
        init_all_tasks_text + strlen(init_all_tasks_text),
    ...
    sprintf(init_all_tasks_text + strlen(init_all_tasks_text),
            "} else if (is_receiver_%d(%s%s,my_rank,nprocs)) { ", loop_num,
            args, params);
    ...
    if (options->variables_not_global) {
      for (i = 0; i < num_data; i++) {
        sprintf(init_all_tasks_text + strlen(init_all_tasks_text), ",%s",
                accs[i]->name);
      }
    }
    sprintf(init_all_tasks_text + strlen(init_all_tasks_text), "));");
    ...
    sprintf(add_outgoing_edges_text,
            "add_outgoing_edges_%d(%s%s%s,tasks_loop%d%s)", loop_num, args,
            params, tasks_loops_args, loop_num, indices);
  ...
  Stmt *init_all_tasks_stmt = create_helper_stmt(
      anchor_stmt, src_copy_level, init_all_tasks_text, ALL_TASKS, loop_num);
  free(init_all_tasks_text);
  for (i = 0; i < src_copy_level;
       i++) { // required for generating iterator declarations
    init_all_tasks_stmt->hyp_types[i] = loop_stmts[0]->hyp_types[i];
  }

  Stmt *add_outgoing_edges_stmt = NULL;
  if (options->dynschedule_graph) {
    add_outgoing_edges_stmt =
        create_helper_stmt(anchor_stmt, src_copy_level, add_outgoing_edges_text,
                           ALL_TASKS, loop_num);
...
void gen_compute_task_cloog_code(PlutoProg *prog, int loop_num,
  ...
  for (i = 0; i < nstmts; i++) {
    ...
    pluto_add_stmt(compute_task, stmt->domain, stmt->trans, stmt->iterators,
                   stmt->text, ORIG_IN_FUNCTION);
    compute_task->stmts[compute_task->nstmts - 1]->first_tile_dim =
        stmt->first_tile_dim;
    compute_task->stmts[compute_task->nstmts - 1]->last_tile_dim =
        stmt->last_tile_dim;
  }

  IF_DEBUG(pluto_prog_print(stdout, compute_task));
  if (compute_task->nstmts >= 1) {
    assert(compute_task->stmts[0]->trans->nrows ==
           compute_task->num_hyperplanes);
  }
...
void gen_data_tile_alloc_cloog_code(PlutoProg *prog, int loop_num,
  ...
  for (i = 0; i < num_data; i++) {
    ...
    PlutoConstraints *data_tiles = get_data_tile_alloc_constraints(
        arr, loop_num, src_copy_level, acc_name, acc_nrows, prog);

    char *alloc_stmt_text = pluto_dist_malloc_stmt_text(acc_name, prog, 1);

    PlutoMatrix *trans =
        get_data_tile_trans_func(src_copy_level, acc_nrows, prog);
  ...
  pluto_separate_stmts(data_tile_prog, data_tile_prog->stmts,
                       data_tile_prog->nstmts, 0, 0);

  IF_DEBUG(pluto_prog_print(stdout, data_tile_prog));

  int temp = options->variables_not_global;
  options->variables_not_global = 0;
  gen_func_cloog_code("data_tile_alloc", headerfp, loop_num, src_copy_level,
                      num_data, accs, data_tile_prog, prog, 0);
...
void gen_data_tile_ref_count_update_cloog_code(PlutoProg *prog, int loop_num,
  ...
  for (i = 0; i < num_data; i++) {
    ...
    PlutoConstraints *data_tiles = get_data_tile_ref_count_update_constraints(
        arr, loop_num, src_copy_level, acc_name, acc_nrows, prog);

    char *ref_count_update_stmt_text =
        get_data_tile_ref_count_stmt_text(arr, prog);
    ...
    pluto_add_stmt(data_tile_prog, data_tiles, trans, iters,
                   ref_count_update_stmt_text, DATA_DIST_MANG);
  ...
  int temp = options->variables_not_global;
  options->variables_not_global = 0;
  gen_func_cloog_code("data_tile_ref_count_update", headerfp, loop_num,
                      src_copy_level, num_data, accs, data_tile_prog, prog, 0);
  options->variables_not_global = temp;
...
void gen_read_in_data_tile_alloc_cloog_code(struct stmt_access_pair **acc_stmts,
  ...
  PlutoConstraints *data_tiles = get_read_in_data_tile_alloc_constraints(
      read_in, arr, loop_num, src_copy_level, acc_name, acc_nrows, prog);
  ...
  int temp = options->variables_not_global;
  options->variables_not_global = 0;
  gen_func_cloog_code(func_name, headerfp, loop_num, src_copy_level, 1, &acc,
                      data_tile_prog, prog, 0);
  options->variables_not_global = temp;
...
void gen_read_in_copy_cloog_code(struct stmt_access_pair **acc_stmts,
  ...
  PlutoConstraints *data_tiles = get_read_in_copy_constraints(
      read_in, arr, loop_num, src_copy_level, acc_name, acc_nrows, prog);
...
void gen_data_tile_ref_count_init_cloog_code(PlutoProg *prog, int loop_num,
  ...
  for (i = 0; i < num_data; i++) {
    ...
    PlutoConstraints *data_tiles = get_data_tile_ref_count_init_constraints(
        arr, loop_num, src_copy_level, acc_name, acc_nrows, prog);

    char *ref_count_update_stmt_text = get_data_tile_ref_count_init_stmt_text(
        arr, src_copy_level, loop_num, prog);
    ...
    pluto_add_stmt(data_tile_prog, data_tiles, trans, iters,
                   ref_count_update_stmt_text, DATA_DIST_INIT);
    ...
    int temp = options->variables_not_global;
    options->variables_not_global = 0;
    gen_func_cloog_code(func_name, headerfp, loop_num, src_copy_level, 1,
                        &accs[i], data_tile_prog, prog, 0);
    options->variables_not_global = temp;
...
get_data_tile_copy_back_constraints(struct stmt_access_pair **wacc_stmts,
  ...
  assert(all_data->ncols ==
         src_copy_level + wacc_stmts[0]->acc->mat->nrows + prog->npar + 1);
...
void gen_data_tile_copy_back_cloog_code(PlutoProg *prog, int loop_num,
  ...
  PlutoConstraints *data_tiles = get_data_tile_copy_back_constraints(
      wacc_stmts, num_accs, src_copy_level, prog);
  ...
  pluto_add_stmt(data_tile_prog, data_tiles, trans, arr->iterators,
                 copy_back_stmt_text, DATA_DIST_COPY);
  ...
  gen_func_cloog_code(func_name, headerfp, loop_num, src_copy_level, num_data,
                      &(wacc_stmts[0]->acc), data_tile_prog, prog, 1);
...
void gen_init_tasks_cloog_code(PlutoProg *prog, Stmt ***tasks_stmts, int nloops,
  ...
  int num_stmts_per_loop =
      (options->dynschedule_graph) ? NUM_GEN_GRAPH_STMTS : NUM_GEN_TASKS_STMTS;
  ...
  if (options->distmem) {
    fprintf(packfp, "int my_rank,int nprocs,int num_threads%s,long int "
                    "*p_num_tasks_to_execute,long int *p_num_tasks_to_unpack",
            tasks_loops_decl);
    fprintf(headerfp, "int my_rank,int nprocs,int num_threads%s,long int "
                      "*p_num_tasks_to_execute,long int *p_num_tasks_to_unpack",
            tasks_loops_decl);
  } else if (options->dynschedule) {
    fprintf(packfp, "int num_threads,long int *p_num_tasks_to_execute%s",
            tasks_loops_decl);
    fprintf(headerfp, "int num_threads,long int *p_num_tasks_to_execute%s",
            tasks_loops_decl);
  } else { // options->dynschedule_graph
    fprintf(packfp, "tbb::flow::graph *__graph, "
                    "tbb::flow::broadcast_node<tbb::flow::continue_msg> "
                    "*__start%s",
            tasks_loops_decl);
    fprintf(headerfp, "tbb::flow::graph *__graph, "
                      "tbb::flow::broadcast_node<tbb::flow::continue_msg> "
                      "*__start%s",
            tasks_loops_decl);
    if (options->variables_not_global) {
...
void gen_write_out_cloog_code(PlutoProg *prog, PlutoProg *write_out_prog,
  ...
  if (write_out_prog->nstmts >= 1) {
    assert(write_out_prog->stmts[0]->trans->nrows ==
           write_out_prog->num_hyperplanes);
  }
...
void gen_dynschedule_graph_main_text_code(PlutoProg *prog, Ploop **loops,
  ...
  for (l = 0; l < nloops; l++) {
    ...
    int num_rw_data;
    PlutoAccess **laccs;
    laccs = pluto_get_all_distinct_vars(loops[l]->stmts, loops[l]->nstmts,
                                        &num_rw_data);
...
void gen_dynschedule_main_text_code(PlutoProg *prog, Ploop **loops, int nloops,
  ...
  fprintf(outfp, "##if defined(__DYNSCHEDULER_DEBUG_PRINT) || "
                 "defined(__DYNSCHEDULER_MORE_DEBUG_PRINT) || "
                 "defined(__DEBUG_FLUSH)\n");
  fprintf(outfp,
          "\tchar *debug_print_name = (char *)malloc(128*sizeof(char));\n");
  fprintf(outfp, "\tstrcpy(debug_print_name, \"debug_print\");\n");
  fprintf(outfp, "\tsprintf(debug_print_name+strlen(debug_print_name), "
                 "\"_node%%d\", my_rank);\n");
  ...
  for (l = 0; l < nloops; l++) {
    wacc_stmts[l] =
        get_write_access_with_stmts(loops[l]->stmts, loops[l]->nstmts,
                                    &num_write_data[l], &num_stmts_per_wacc[l]);
  }

  int max_copy_level = copy_level[0];
  for (l = 1; l < nloops; l++) {
    if (copy_level[l] > max_copy_level)
      max_copy_level = copy_level[l];
  }
  ...
  if (options->distmem) {
    fprintf(outfp, "\tinit_tasks(my_rank,nprocs,_num_threads%s,&_num_tasks_to_"
                   "execute,&_num_tasks_to_unpack);\n",
            tasks_loops_args);
  } else {
  ...
  if (options->distmem) {
    ...
    for (i = 0; i < num_data; i++) {
      fprintf(outfp, "\t\t__error = MPI_Irecv(recv_buf_%s[__p], %s, MPI_DOUBLE,\
                MPI_ANY_SOURCE, %s, MPI_COMM_WORLD, &recv_reqs_%s[__p]);\n",
      ...
      fprintf(outfp, "\t\t\tchar "
                     "*msg=(char*)malloc((MPI_MAX_ERROR_STRING+1)*sizeof(char))"
                     "; int len;\n");
      fprintf(outfp, "\t\t\tMPI_Error_string(__error, msg, &len);\n");
      fprintf(outfp, "\t\t\tfprintf(__debug_print_fp, \"node %%d %s recv buf "
    ...
    fprintf(outfp, "##ifdef __DYNSCHEDULER_DEBUG_PRINT\n");
    fprintf(outfp, "\tfprintf(__debug_print_fp, \"node %%d num tasks to "
                   "execute %%ld num tasks to unpack %%ld max recv buffers "
                   "%%d");
    for (i = 0; i < num_data; i++) {
      fprintf(outfp, " %s max elements %%d", waccs[i]->name);
    }
    fprintf(outfp, "\\n\", my_rank, _num_tasks_to_execute, "
                   "_num_tasks_to_unpack, __MAX_NUM_RECVS");
    for (i = 0; i < num_data; i++) {
  ...
  if (options->distmem) {
    ...
    for (i = 0; i < num_data; i++) {
      ...
      fprintf(outfp, "\t\t\t\tMPI_Test(&recv_reqs_%s[__p],&recv_completed_%s[__"
                     "p],MPI_STATUS_IGNORE);\n",
              acc_names[i], acc_names[i]);
      fprintf(outfp, "##else\n");
      fprintf(outfp, "\t\t\t\tMPI_Status recv_stats_%s;\n", acc_names[i]);
      fprintf(outfp, "\t\t\t\trecv_stats_%s.MPI_ERROR = "
                     "MPI_Test(&recv_reqs_%s[__p],&recv_completed_%s[__p],&"
                     "recv_stats_%s);\n",
      ...
      fprintf(outfp,
              "\t\t\t\t\tMPI_Get_count(&recv_stats_%s, MPI_DOUBLE, &size); \n",
              acc_names[i]);
      fprintf(outfp,
              "\t\t\t\t\tif (recv_stats_%s.MPI_ERROR != MPI_SUCCESS) {\n",
              acc_names[i]);
      fprintf(outfp, "\t\t\t\t\t\tchar "
                     "*msg=(char*)malloc((MPI_MAX_ERROR_STRING+1)*sizeof(char))"
                     "; int len;\n");
      fprintf(
          outfp,
          "\t\t\t\t\t\tMPI_Error_string(recv_stats_%s.MPI_ERROR, msg, &len);\n",
          acc_names[i]);
      fprintf(outfp, "\t\t\t\t\t\tfprintf(__debug_print_fp, \"received %s node "
                     "%%d from node %%d buf %%d size %%d loop %%d task",
              acc_names[i]);
      for (j = 0; j < max_copy_level; j++) {
        fprintf(outfp, " %%d");
      }
      fprintf(outfp, " error %%d %%s\\n\", my_rank, recv_stats_%s.MPI_SOURCE, "
                     "__p, size, __loop",
              acc_names[i]);
      for (j = 0; j < max_copy_level; j++) {
      ...
      fprintf(outfp,
              "\\n\", my_rank, recv_stats_%s.MPI_SOURCE, __p, size, __loop",
              acc_names[i]);
      for (j = 0; j < max_copy_level; j++) {
        fprintf(outfp, ", task_id%d", j + 1);
      }
        ...
        for (j = 0; j < num_write_data[l]; j++) {
          if (!strcmp(wacc_stmts[l][j][0]->acc->name, acc_names[i])) {
            ...
            for (ii = 0; ii < num_dims; ii++) {
              sprintf(indices + strlen(indices),
                      "(task_id%d-lb_tasks_loop%d_dim%d)", dims[ii] + 1, l, ii);
              for (k = ii + 1; k < num_dims; k++) {
                sprintf(indices + strlen(indices),
                        "*max_num_tasks_loop%d_dim%d", l, k);
              }
              strcat(indices, "+");
            }
            strcat(indices, "0]");
            ...
            /* make it src_copy_level+1 since we use an extra dimension to
             * separate
             * statements */
            ...
            fprintf(outfp, "##ifdef __DYNSCHEDULER_MORE_DEBUG_PRINT\n");
            fprintf(outfp, "\t\t\t\t\t\tif (prev_firing_count_%s_%d[__p] != "
                           "tasks_loop%d%s) {\n",
                    acc_names[i], l, l, indices);
            fprintf(outfp, "\t\t\t\t\t\t\tprev_firing_count_%s_%d[__p] = "
                           "tasks_loop%d%s;\n",
                    acc_names[i], l, l, indices);
            fprintf(outfp, "\t\t\t\t\t\t\tfprintf(__debug_print_fp, \"waiting "
                           "to unpack %s node %%d buf %%d loop %%d task",
                    acc_names[i]);
            for (j = 0; j < max_copy_level; j++) {
              fprintf(outfp, " %%d");
            }
            ...
            fprintf(outfp, "\t\t\t\t\t\tif (tasks_loop%d%s <= %d) {\n", l,
                    indices, num_write_data[l]);

            if (options->timereport) {
              fprintf(outfp, "\t\t\t\t\t\t\tIF_TIME(t_wait_unpack += rtclock() "
                             "- t_wait_unpack_start);\n");
              fprintf(outfp,
            ...
            fprintf(outfp, "\t\t\t\t\t\t\tif (__error) {\n");
            fprintf(outfp, "\t\t\t\t\t\t\t\tchar "
                           "*msg=(char*)malloc((MPI_MAX_ERROR_STRING+1)*sizeof("
                           "char)); int len;\n");
            fprintf(outfp,
                    "\t\t\t\t\t\t\t\tMPI_Error_string(__error, msg, &len);\n");
            fprintf(outfp, "\t\t\t\t\t\t\t\tfprintf(__debug_print_fp, \"node "
                           "%%d %s recv buf %%d error %%d %%s\\n\", my_rank, "
                           "__p, __error, msg);\n",
                    acc_names[i]);
            fprintf(outfp, "\t\t\t\t\t\t\t\tfree(msg);\n");
            fprintf(outfp, "\t\t\t\t\t\t\t}\n");
            ...
            fprintf(outfp, "\t\t\t\t\t\t\tfprintf(__debug_print_fp, \"unpacked "
                           "%s node %%d buf %%d loop %%d task",
                    acc_names[i]);
            for (j = 0; j < max_copy_level; j++) {
              fprintf(outfp, " %%d");
            }
            ...
            for (k = 0; k < copy_level[l] + prog->npar; k++) {
              if (k != 0)
                fprintf(outfp, ",");
              if (k <= copy_level[l] - 1)
                fprintf(outfp, "task_id%d", k + 1);
              else
                fprintf(outfp, "%s", prog->params[k - copy_level[l]]);
            }
            // my_rank is already added as a parameter
            fprintf(outfp, ",nprocs,_num_threads%s);\n", tasks_loops_args);
            fprintf(outfp, "\t\t\t\t\t\t\t\t_num_tasks_to_unpack--;\n");
            fprintf(outfp, "##ifdef __DYNSCHEDULER_DEBUG_PRINT\n");
            fprintf(outfp, "\t\t\t\t\t\t\t\tfprintf(__debug_print_fp, \"remote "
                           "updated %s node %%d loop %%d task",
                    acc_names[i]);
            for (j = 0; j < max_copy_level; j++) {
              fprintf(outfp, " %%d");
            }
            fprintf(outfp,
                    " num tasks left to unpack %%ld\\n\", my_rank, __loop");
            for (j = 0; j < max_copy_level; j++) {
              fprintf(outfp, ", task_id%d", j + 1);
            }
            fprintf(outfp, ", _num_tasks_to_unpack);\n");
            ...
            fprintf(outfp, "\t\t\t\t\t\t\t}\n");
            if (options->timereport) {
              fprintf(outfp, "\t\t\t\t\t\t\tIF_TIME(t_unpack += rtclock() - "
                             "t_unpack_start);\n");
              fprintf(
                  outfp,
                  "\t\t\t\t\t\t\tIF_TIME(t_wait_unpack_start = rtclock());\n");
            }
      ...
      fprintf(outfp, "\t\t\t\t\t\tfprintf(stderr, \"ERROR: %s data cannot be "
                     "sent by a task of loop %%d\\n\", __loop);\n",
              acc_names[i]);
    ...
    for (i = 0; i < num_data; i++) {
      fprintf(outfp, "\t\tfor (__p=0; __p<recv_buf_%s.size(); __p++) {\n",
              acc_names[i]);
      fprintf(outfp,
              "\t\t\tif (recv_reqs_%s[__p] != MPI_REQUEST_NULL) break;\n",
              acc_names[i]);
      fprintf(outfp, "\t\t}\n");
      fprintf(outfp, "\t\tif (__p == recv_buf_%s.size()) {\n", acc_names[i]);
      fprintf(outfp, "\t\t\tfor (__p=0; __p<__MAX_NUM_RECVS; __p++) {\n");
      ...
      fprintf(outfp, "\t\t\t\tbuf = (double *) polyrt_max_alloc(buf, "
                     "sizeof(double)*%s, &buf_size);\n",
              max_elements_name[i]);
      ...
      fprintf(outfp, "##endif\n");
      fprintf(outfp, "\t\t\t\tMPI_Irecv(buf, %s, MPI_DOUBLE,\
                MPI_ANY_SOURCE, %s, MPI_COMM_WORLD, &recv_reqs_%s[recv_reqs_%s.size()-1]);\n",
              max_elements_name[i], data_tag[i], acc_names[i], acc_names[i]);
      fprintf(outfp, "\t\t\t}\n");
      fprintf(outfp, "##ifdef __DYNSCHEDULER_DEBUG_PRINT\n");
      fprintf(outfp, "\t\t\tfprintf(__debug_print_fp, \"node %%d recv thread "
                     "%%d %s max recv buffers %%lu\\n\", my_rank, __tid, "
                     "recv_buf_%s.size());\n",
              acc_names[i], acc_names[i]);
      fprintf(outfp, "\t\t\tIF_DEBUG_FLUSH(fflush(__debug_print_fp));\n");
      fprintf(outfp, "##endif\n");
  ...
  for (section = 0; section < 2; section++) {
    ...
    for (i = 0; i < num_data; i++) {
      fprintf(outfp,
              "\t\t\tfor (__i=0;__i<send_reqs_%s[__tid].size();__i++) {\n",
              acc_names[i]);
      fprintf(outfp,
              "\t\t\t\tif (send_reqs_%s[__tid][__i] != MPI_REQUEST_NULL) {\n",
              acc_names[i]);
      fprintf(outfp, "\t\t\t\tint flag, __error = "
                     "MPI_Test(&send_reqs_%s[__tid][__i], &flag, "
                     "MPI_STATUS_IGNORE);\n",
              acc_names[i]);
      fprintf(outfp, "\t\t\t\t\tif (__error) {\n");
      fprintf(outfp, "\t\t\t\t\t\tchar "
                     ...
                     "; int len;\n");
      fprintf(outfp, "\t\t\t\t\t\tMPI_Error_string(__error, msg, &len);\n");
      fprintf(outfp, "\t\t\t\t\t\tfprintf(__debug_print_fp, \"node %%d %s send "
                     "test buf %%d error %%d %%s\\n\", my_rank, __i, __error, "
                     "msg);\n",
              acc_names[i]);
      fprintf(outfp, "\t\t\t\t\t\tfree(msg);\n");
      fprintf(outfp, "\t\t\t\t\t}\n");
    ...
    fprintf(outfp, "\t\tfprintf(__debug_print_fp, \"computing node %%d thread "
                   "%%d loop %%d task");
    for (j = 0; j < max_copy_level; j++) {
      fprintf(outfp, " %%d");
    }
    ...
    for (l = 0; l < nloops; l++) {
      ...
      for (ii = 0; ii < num_dims; ii++) {
        sprintf(indices + strlen(indices), "(task_id%d-lb_tasks_loop%d_dim%d)",
                dims[ii] + 1, l, ii);
        for (k = ii + 1; k < num_dims; k++) {
          sprintf(indices + strlen(indices), "*max_num_tasks_loop%d_dim%d", l,
                  k);
        }
        strcat(indices, "+");
      }
      ...
      int num_rw_data;
      PlutoAccess **accs;
      accs = pluto_get_all_distinct_vars(loops[l]->stmts, loops[l]->nstmts,
                                         &num_rw_data);

      if (options->timereport) {
        fprintf(outfp, "\t\t\t\tIF_TIME(t_wait_comp[__tid] += rtclock() - "
                       "t_wait_comp_start[__tid]);\n");

        if (options->data_dist) {
          fprintf(outfp,
                  "\t\t\t\tIF_TIME(t_data_mang_start[__tid] = rtclock());\n");
          ...
          fprintf(outfp, ");\n");
          fprintf(outfp, "\t\t\t\tIF_TIME(t_data_mang[__tid] += rtclock() - "
                         "t_data_mang_start[__tid]);\n");
        }
      ...
      fprintf(outfp, ");\n");
      if (options->timereport)
        fprintf(outfp, "\t\t\t\tIF_TIME(t_comp[__tid] += rtclock() - "
                       "t_comp_start[__tid]);\n");
      ...
      if (options->distmem) {
        ...
        fprintf(outfp, ");\n");
        if (options->timereport)
          fprintf(outfp, "\t\t\t\tIF_TIME(t_pack[__tid] += rtclock() - "
                         "t_pack_start[__tid]);\n");
      }

      if (options->timereport)
        fprintf(outfp,
                "\t\t\t\tIF_TIME(t_tasks_manage_start[__tid] = rtclock());\n");
      ...
      for (i = 0; i < copy_level[l] + prog->npar; i++) {
        ...
        if (i <= copy_level[l] - 1)
          ...
          fprintf(outfp, "%s", prog->params[i - copy_level[l]]);
      }
      if (options->distmem) {
        // my_rank is already added as a parameter
      ...
      fprintf(outfp, "%s);\n", tasks_loops_args);
      if (options->timereport) {
        fprintf(outfp, "\t\t\t\tIF_TIME(t_tasks_manage[__tid] += rtclock() - "
                       "t_tasks_manage_start[__tid]);\n");

        fprintf(outfp,
                "\t\t\t\tIF_TIME(t_wait_comp_start[__tid] = rtclock());\n");
      }
      fprintf(outfp, "\t\t\t\tbreak;\n");
    ...
    fprintf(outfp, "\t\t}\n");
    fprintf(outfp, "##ifdef __DYNSCHEDULER_DEBUG_PRINT\n");
    fprintf(outfp, "\t\tfprintf(__debug_print_fp, \"computed node %%d thread "
                   "%%d loop %%d task");
    for (j = 0; j < max_copy_level; j++) {
      fprintf(outfp, " %%d");
    }
...
void gen_dynschedule_graph_header_text_code(PlutoProg *prog, int *copy_level,
  ...
  for (l = 0; l < nloops; l++) {
    for (i = 0; i < num_distinct_copy_level; i++) {
      if (copy_level[l] == distinct_copy_level[i])
        break;
    }
  ...
  if (options->variables_not_global) {
    for (i = 0; i < num_data; i++) {
      fprintf(headerfp, "__DECLARATION_OF_POINTER_TO_%s(%s);", accs[i]->name,
              accs[i]->name);
    }
  }
  ...
  for (l = 0; l < num_distinct_copy_level; l++) {
    ...
    if (options->variables_not_global) {
      for (i = 0; i < num_data; i++) {
        fprintf(headerfp, ", __DECLARATION_OF_POINTER_TO_%s(_%s)",
...
void gen_dynschedule_header_text_code(int *copy_level, int nloops,
  ...
  fprintf(headerfp, "\tpublic:\n");
  fprintf(headerfp,
          "\t\t// returns true only if task2 has higher priority than task1\n");
  fprintf(headerfp, "\t\tbool operator()(const __Task &task1, const __Task "
                    "&task2) const {\n");
  fprintf(headerfp, "#ifndef __DYNSCHEDULER_NO_PRIORITY\n");
  if (options->distmem) {
    fprintf(headerfp, "\t\t\tif "
                      "(task2.remote_dep_tasks>task1.remote_dep_tasks) return "
                      "true;\n");
    fprintf(headerfp, "\t\t\tif "
                      "(task1.remote_dep_tasks>task2.remote_dep_tasks) return "
                      "false;\n");
  }
  fprintf(
      headerfp,
      "\t\t\tif (task2.local_dep_tasks>task1.local_dep_tasks) return true;\n");
  fprintf(
      headerfp,
      "\t\t\tif (task1.local_dep_tasks>task2.local_dep_tasks) return false;\n");
  fprintf(headerfp, "\t\t\tint thread_id = omp_get_thread_num();\n");
  fprintf(headerfp, "\t\t\tif ((thread_id == task2.thread_affinity) && "
                    "(thread_id != task1.thread_affinity)) return true;\n");
  fprintf(headerfp, "\t\t\tif ((thread_id == task1.thread_affinity) && "
                    "(thread_id != task2.thread_affinity)) return false;\n");
  ...
  fprintf(
      headerfp,
      "extern tbb::concurrent_priority_queue<__Task, __Compare> pqueue;\n\n");
  fprintf(headerfp,
          "tbb::concurrent_priority_queue<__Task, __Compare> pqueue;\n");
}
...
Stmt **gen_comm_code_opt_fop(int data_id, struct stmt_access_pair **wacc_stmts,
  ...
  PlutoConstraintsList *atomic_flowouts =
      pluto_constraints_list_alloc((PlutoConstraints *)NULL);
  PlutoConstraints *flow_out =
      NULL; // used only for get parametric extent/bounding box
  for (k = 0; k < num_accs; k++) {
    PlutoConstraints *flow_out_one;
    flow_out_one = compute_flow_out(wacc_stmts[k], src_copy_level, copy_level,
                                    prog, pi_mappings);
    if (flow_out == NULL)
      flow_out = pluto_constraints_dup(flow_out_one);
    else {
      pluto_constraints_unionize(flow_out, flow_out_one);
    }
    pluto_constraints_free(flow_out_one);

    compute_flow_out_partitions(wacc_stmts[k], src_copy_level, copy_level, prog,
                                atomic_flowouts, pi_mappings);
  }

  PlutoConstraintsList *curr;
  if (outer_dist_loop_level ==
      (src_copy_level - 1)) { // implies !multi_level_distribution
  ...
  generate_sigma_dep_split(wacc_stmts, num_accs, copy_level, prog,
                           atomic_flowouts, loop_num, pi_mappings, sigmafp,
                           headerfp);
  ...
  if (options->dynschedule) { // one more for the loop number
    sprintf(prog->decls + strlen(prog->decls), "\
            %s = max(%s, %s + %d);\n",
            max_elements_name, max_elements_name, send_buf_size,
            max_copy_level + 1);
    sprintf(prog->decls + strlen(prog->decls), "\
  ...
  char *flow_copy_text =
      malloc(strlen(sendbufname) + strlen("[") + strlen(send_counts_name) +
             strlen("++] = ") + strlen(access) + 1);
  sprintf(flow_copy_text, "%s[%s++] = %s", sendbufname, send_counts_name,
          access);
  ...
  if (options->dynschedule) {
    ...
    sprintf(prog->decls + strlen(prog->decls),
            "%s = (double *) polyrt_max_alloc(%s, nprocs*send_buf_size_%s[0], "
            "&recv_buf_size_%s);\n",
  ...
  if (options->dynschedule) {
    ...
    sprintf(comm_text + strlen(comm_text), "\
				...
				if(%s[__p] > 0) {",
            send_counts_name);
    sprintf(comm_text + strlen(comm_text), "%s[index_%s[__p]][0] = %d;",
            sendbufname, acc_name, loop_num);
    for (i = 0; i < src_copy_level; i++) {
      sprintf(comm_text + strlen(comm_text), "%s[index_%s[__p]][%d] = t%d;",
              sendbufname, acc_name, i + 1, i + 1);
    }
    sprintf(comm_text + strlen(comm_text), "\
    ...
    if (options->timereport)
      sprintf(comm_text + strlen(comm_text),
              "IF_TIME(__total_count += %s[__p]);", send_counts_name);
    sprintf(comm_text + strlen(comm_text), "\
            ...
            sendbufname, acc_name, send_counts_name, data_tag, acc_name,
            acc_name, send_counts_name, acc_name, acc_name);
  } else {
    if (options->timereport)
  ...
  char *flow_copyback_text =
      malloc(strlen(access) + strlen(" = ") + strlen(recvbufname) +
             strlen("[") + strlen(currdisplsname) + strlen("++]") + 1);
  sprintf(flow_copyback_text, "%s = %s[%s++]", access, recvbufname,
          currdisplsname);

  /* Create statement stub to get hold of the outer loops for the copy
   * stmt; rest of the loops to be added are the actual copy loops */
  Stmt *comm_stmt = create_helper_stmt(anchor_stmt, outer_dist_loop_level,
                                       comm_text, COMM_CALL, loop_num);
  ...
  if (options->dynschedule) {
    ...
    sprintf(unpack_stmt_text + strlen(unpack_stmt_text), "%s = %d; ",
            currdisplsname, max_copy_level + 1);
    if (options->fop_unicast_runtime) {
      sprintf(unpack_stmt_text + strlen(unpack_stmt_text), "int __p = pi_%d(",
              loop_num);
      for (i = 0; i < src_copy_level; i++) {
        if (i != 0) {
          sprintf(unpack_stmt_text + strlen(unpack_stmt_text), ",");
        }
        sprintf(unpack_stmt_text + strlen(unpack_stmt_text), "(int)%s[%d]",
                recvbufname, i + 1);
      }
      sprintf(unpack_stmt_text + strlen(unpack_stmt_text), "%s, nprocs); ",
              params);
    }
  } else {
    sprintf(unpack_stmt_text, "proc = pi_%d(%s%s, nprocs); ", loop_num, args,
            params);
    sprintf(unpack_stmt_text + strlen(unpack_stmt_text),
            "if ((my_rank != proc) && (%s[proc] > 0)) { ", recv_counts_name);
  }
  k = 0;
  curr = atomic_flowouts;
  while (curr != NULL) {
    if ((curr->constraints == NULL) ||
        pluto_constraints_is_empty(curr->constraints)) {
      curr = curr->next;
      continue;
    }
    ...
    if (options->fop_unicast_runtime) {
      ...
      while (dep_list != NULL) {
        ...
        PlutoConstraints *fi =
            compute_flow_in_of_dep(dep, dest_copy_level, prog, 1);
        PlutoConstraints *rt = get_receiver_tiles_of_dep(
            dep, src_copy_level, dest_copy_level, prog, 1);
      ...
      if (!broadcast) {
        ...
        sprintf(sigma_check_text, "distinct_recv = sigma_check_%s_%d(%s%s",
                acc_name_k, loop_num, args, params);

        sprintf(pack_stmt_text + strlen(pack_stmt_text),
                "%s, my_rank, nprocs); if (distinct_recv == 1) {",
                sigma_check_text);
        if (options->dynschedule) {
          sprintf(unpack_stmt_text + strlen(unpack_stmt_text),
                  "distinct_recv = sigma_check_%s_%d(", acc_name_k, loop_num);
          for (i = 0; i < src_copy_level; i++) {
            if (i != 0) {
              sprintf(unpack_stmt_text + strlen(unpack_stmt_text), ",");
            }
          ...
          sprintf(unpack_stmt_text + strlen(unpack_stmt_text), "%s,",
                  sigma_check_text);
        }
        if (options->dynschedule) {
          sprintf(unpack_stmt_text + strlen(unpack_stmt_text),
                  " __p, nprocs); if (distinct_recv == 1) {");
        } else {
          sprintf(unpack_stmt_text + strlen(unpack_stmt_text),
                  " proc, nprocs); if (distinct_recv == 1) {");
        }
        ...
        for (l = 0; l < nloops; l++) {
          ...
          sprintf(decl_dest_args + strlen(decl_dest_args), "int ts%d",
                  src_copy_level + 1);
          for (i = src_copy_level + 1; i < total_copy_level; i++) {
            sprintf(decl_dest_args + strlen(decl_dest_args), ",int ts%d",
                    i + 1);
          ...
          char pi_text[1024];
          sprintf(pi_text, "recv_proc = pi_%d(%s%s, nprocs)", l, dest_args,
                  params);

          if (options->dynschedule) {
            sprintf(pack_stmt_text + strlen(pack_stmt_text),
                    "pack_foifi_%s_%d_%d(%s,%s,%s, my_rank, nprocs, "
                    "send_reqs_%s, index_%s, &current_index_%s",
                    acc_name_k, loop_num, l, args, sendbufname,
                    send_counts_name, acc_name, acc_name, acc_name);
          } else {
            sprintf(pack_stmt_text + strlen(pack_stmt_text),
                    "pack_foifi_%s_%d_%d(%s,%s,%s, my_rank, nprocs", acc_name_k,
                    loop_num, l, args, sendbufname, send_counts_name);
          }
          if (options->variables_not_global && !options->data_dist)
            sprintf(pack_stmt_text + strlen(pack_stmt_text), ",%s", acc_name);
          print_data_dist_parm_call(pack_stmt_text, arr);

          sprintf(pack_stmt_text + strlen(pack_stmt_text), ");");

          char flow_cg_text[2048];
          sprintf(flow_cg_text, "%s; if (recv_proc != my_rank) { ", pi_text);
          if (options->dynschedule) {
            sprintf(flow_cg_text + strlen(flow_cg_text), "\
                    ...
                    send_counts_name, acc_name, acc_name, acc_name, sendbufname,
                    acc_name);
            sprintf(flow_cg_text + strlen(flow_cg_text), "\
								%s[recv_proc] = %d; }",
                    send_counts_name,
                    max_copy_level + 1); // one more for the loop number
          }
          if (options->dynschedule) {
            sprintf(flow_cg_text + strlen(flow_cg_text), "\
                                %s[recv_proc] = pack_foifi_recv_%s_%d_%d(%s,%s,%s[index[recv_proc]],%s[recv_proc]",
                    send_counts_name, acc_name_k, loop_num, l, passed_args,
                    dest_args, sendbufname, send_counts_name);
          } else {
            sprintf(flow_cg_text + strlen(flow_cg_text), "\
          ...
          if (options->variables_not_global && !options->data_dist)
            sprintf(flow_cg_text + strlen(flow_cg_text), ",%s", acc_name);
          ...
          if (options->dynschedule) {
            fprintf(
                packfp,
                "int pack_foifi_%s_%d_%d(%s,std::vector<double *>&%s,int *%s, \
								int my_rank, int nprocs, std::vector<MPI_Request>& send_reqs",
                acc_name_k, loop_num, l, decl_args, sendbufname,
                send_counts_name);
            fprintf(
                headerfp,
                "int pack_foifi_%s_%d_%d(%s,std::vector<double *>&%s,int *%s, \
          ...
          generate_pack_or_unpack(packfp, prog, receiver_tiles, flow_cg_text,
                                  IN_FUNCTION, dest_iters, src_copy_level,
                                  dest_copy_level, acc_name, send_counts_name);

          fprintf(packfp,
                  "int pack_foifi_recv_%s_%d_%d(%s,%s,double *%s,int %s",
                  acc_name_k, loop_num, l, decl_args, decl_dest_args,
                  sendbufname, send_counts_name);
          fprintf(headerfp,
                  "int pack_foifi_recv_%s_%d_%d(%s,%s,double *%s,int %s",
                  acc_name_k, loop_num, l, decl_args, decl_dest_args,
          ...
          add_data_dist_parm_decl(headerfp, acc_name, prog);
          fprintf(headerfp, ");\n");
          generate_pack_or_unpack(packfp, prog, foifi, flow_copy_text,
                                  IN_FUNCTION, iters, total_copy_level,
                                  acc_nrows, acc_name, send_counts_name);

          if (options->dynschedule) {
            sprintf(unpack_stmt_text + strlen(unpack_stmt_text),
                    "%s = unpack_foifi_%s_%d_%d(", currdisplsname, acc_name_k,
                    loop_num, l);
            for (i = 0; i < src_copy_level; i++) {
              sprintf(unpack_stmt_text + strlen(unpack_stmt_text),
                      "(int)%s[%d],", recvbufname, i + 1);
            }
            sprintf(unpack_stmt_text + strlen(unpack_stmt_text),
                    "%s,%s, my_rank, nprocs", recvbufname, currdisplsname);
          } else {
            sprintf(unpack_stmt_text + strlen(unpack_stmt_text),
                    "%s[proc] = unpack_foifi_%s_%d_%d(%s,%s,%s[proc], my_rank, "
                    "nprocs",
                    currdisplsname, acc_name_k, loop_num, l, args, recvbufname,
                    currdisplsname);
          }
          if (options->variables_not_global && !options->data_dist)
            sprintf(unpack_stmt_text + strlen(unpack_stmt_text), ",%s",
          ...
          sprintf(flow_copyback_guard_text, "%s; if (recv_proc == my_rank) \
                  ...
                  pi_text, currdisplsname, acc_name_k, loop_num, l, passed_args,
                  dest_args, recvbufname, currdisplsname);
          if (options->variables_not_global && !options->data_dist)
          ...
          if (options->data_dist) {
            ...
            sprintf(flow_copyback_guard_text + strlen(flow_copyback_guard_text),
                    ", %s_trans", arr->text);
            for (s = 1; s < arr->num_tiled_loops; ++s) {
              sprintf(flow_copyback_guard_text +
                          strlen(flow_copyback_guard_text),
                      ", %s_size_%d", arr->text, s + arr->first_tile_dim);
            }
          ...
          fprintf(packfp, "int unpack_foifi_%s_%d_%d(%s,double *%s,int %s, int "
                          "my_rank, int nprocs",
                  acc_name_k, loop_num, l, decl_args, recvbufname,
                  currdisplsname);
          fprintf(headerfp, "int unpack_foifi_%s_%d_%d(%s,double *%s,int %s, "
                            "int my_rank, int nprocs",
                  acc_name_k, loop_num, l, decl_args, recvbufname,
                  currdisplsname);
          if (options->variables_not_global && !options->data_dist) {
          ...
          add_data_dist_parm_decl(headerfp, acc_name, prog);
          fprintf(headerfp, ");\n");
          generate_pack_or_unpack(packfp, prog, receiver_tiles,
                                  flow_copyback_guard_text, IN_FUNCTION,
                                  dest_iters, src_copy_level, dest_copy_level,
                                  acc_name, currdisplsname);

          fprintf(packfp,
                  "int unpack_foifi_recv_%s_%d_%d(%s,%s,double *%s,int %s",
                  acc_name_k, loop_num, l, decl_args, decl_dest_args,
          ...
          add_data_dist_parm_decl(headerfp, acc_name, prog);
          fprintf(headerfp, ");\n");
          generate_pack_or_unpack(packfp, prog, foifi, flow_copyback_text,
                                  IN_FUNCTION, iters, total_copy_level,
    ...
    // !!!roshan should add the below keyword to parallelize per-receiver copy
    // loop
    ...
    if (options->dynschedule) {
      sprintf(pack_stmt_text + strlen(pack_stmt_text), "\
              ...
              send_counts_name, acc_name, acc_name, acc_name, acc_name,
              acc_name, acc_name, acc_name, acc_name, acc_name, acc_name,
              acc_name, acc_name, acc_name, acc_name, sendbufname, acc_name,
              acc_name, acc_name, acc_name, acc_name);
      sprintf(pack_stmt_text + strlen(pack_stmt_text), "\
					%s[__p] = %d; }",
              send_counts_name,
              max_copy_level + 1); // one more for the loop number
      sprintf(pack_stmt_text + strlen(pack_stmt_text),
              ...
              send_counts_name, acc_name_k, loop_num, args, sendbufname,
              acc_name, send_counts_name);
    } else {
      sprintf(pack_stmt_text + strlen(pack_stmt_text),
              "%s[__p] = pack_%s_%d(%s, %s[__p], %s[__p]", send_counts_name,
              acc_name_k, loop_num, args, sendbufname, send_counts_name);
    }
    if (options->variables_not_global && !options->data_dist)
      sprintf(pack_stmt_text + strlen(pack_stmt_text), ",%s", acc_name);
    ...
    if (options->fop_unicast_runtime && !broadcast)
      sprintf(pack_stmt_text + strlen(pack_stmt_text), "}");

    if (curr != NULL && curr->deps == NULL) {
      curr->constraints =
          pluto_constraints_empty(src_copy_level + acc_nrows + prog->npar + 1);
    }
    assert(curr->constraints->ncols ==
           src_copy_level + acc_nrows + prog->npar + 1);

    fprintf(packfp, "int pack_%s_%d(%s,double *%s,int %s", acc_name_k, loop_num,
            decl_args, sendbufname, send_counts_name);
    fprintf(headerfp, "int pack_%s_%d(%s,double *%s,int %s", acc_name_k,
            loop_num, decl_args, sendbufname, send_counts_name);
    if (options->variables_not_global && !options->data_dist) {
      fprintf(packfp, ", __DECLARATION_OF_%s", acc_name);
      fprintf(headerfp, ", __DECLARATION_OF_%s", acc_name);
    }
    add_data_dist_parm_decl(headerfp, acc_name, prog);
    fprintf(headerfp, ");\n");
    generate_pack_or_unpack(packfp, prog, curr->constraints, flow_copy_text,
                            IN_FUNCTION, iters, src_copy_level, acc_nrows,
                            acc_name, send_counts_name);

    if (options->dynschedule) {
      sprintf(unpack_stmt_text + strlen(unpack_stmt_text),
              "if (is_receiver_%s_%d(", acc_name_k, loop_num);
      for (i = 0; i < src_copy_level; i++) {
        if (i != 0) {
          sprintf(unpack_stmt_text + strlen(unpack_stmt_text), ",");
        }
      ...
      sprintf(unpack_stmt_text + strlen(unpack_stmt_text), "%s,%s", recvbufname,
              currdisplsname);
    } else {
      sprintf(unpack_stmt_text + strlen(unpack_stmt_text),
              "if (is_receiver_%s_%d(%s%s,my_rank,nprocs) != 0) { \
				%s[proc] = unpack_%s_%d(%s,%s,%s[proc]",
              acc_name_k, loop_num, args, params, currdisplsname, acc_name_k,
              loop_num, args, recvbufname, currdisplsname);
    }
    if (options->variables_not_global && !options->data_dist)
      sprintf(unpack_stmt_text + strlen(unpack_stmt_text), ",%s", acc_name);
    ...
    fprintf(packfp, "int unpack_%s_%d(%s,double *%s,int %s", acc_name_k,
            loop_num, decl_args, recvbufname, currdisplsname);
    fprintf(headerfp, "int unpack_%s_%d(%s,double *%s,int %s", acc_name_k,
            loop_num, decl_args, recvbufname, currdisplsname);
    if (options->variables_not_global && !options->data_dist) {
      fprintf(packfp, ", __DECLARATION_OF_%s", acc_name);
      fprintf(headerfp, ", __DECLARATION_OF_%s", acc_name);
    ...
    add_data_dist_parm_decl(headerfp, acc_name, prog);
    fprintf(headerfp, ");\n");
    generate_pack_or_unpack(packfp, prog, curr->constraints, flow_copyback_text,
                            IN_FUNCTION, iters, src_copy_level, acc_nrows,
                            acc_name, currdisplsname);
...
Stmt **gen_comm_code_opt_foifi(int data_id,
  ...
  for (k = 0; k < num_accs; k++) {
    ...
    for (i = 0; i < prog->ndeps; i++) {
      ...
      if (options->data_dist) {
        ...
        if (dep->type != OSL_DEPENDENCE_RAW && dep->type != OSL_DEPENDENCE_RAR)
          continue;
      } else {
      ...
      if (dep->src_acc != wacc_stmts[k]->acc)
        continue;
      ...
      PlutoConstraints *fo = compute_flow_out_of_dep(
          dep, src_copy_level, copy_level, prog, 0, NULL, pi_mappings);
      // add target tile iterators for flow-out set
      for (j = 0; j < dest_copy_level; j++) {
        pluto_constraints_add_dim(fo, src_copy_level, NULL);
      }
      ...
      PlutoConstraints *fi =
          compute_flow_in_of_dep(dep, dest_copy_level, prog, 0);
      // add source tile iterators for flow-in set
      for (j = 0; j < src_copy_level; j++) {
  ...
  // === from here on we begin the code generation common for all dependent
  // loops===================================
  //

  // !!!roshan may need to increase send buffer size to an arbitrarily large
  // number
  ...
  if (broadcast) {
    if ((flow_out != NULL) && !pluto_constraints_is_empty(flow_out)) {
      ...
      if (options->dynschedule) {
        ...
        sprintf(pack_stmt_text + strlen(pack_stmt_text),
                "%s[__p] = pack_%s_%d(%s,%s[index_%s[__p]],%s[__p]",
                send_counts_name, acc_name, loop_num, args, sendbufname,
                acc_name, send_counts_name);
      } else {
        sprintf(pack_stmt_text + strlen(pack_stmt_text),
                "%s[__p] = pack_%s_%d(%s,%s[__p],%s[__p]", send_counts_name,
                acc_name, loop_num, args, sendbufname, send_counts_name);
      }
      if (options->variables_not_global && !options->data_dist)
        sprintf(pack_stmt_text + strlen(pack_stmt_text), ",%s", acc_name);
      print_data_dist_parm_call(pack_stmt_text, arr);
      ...
      fprintf(packfp, "int pack_%s_%d(%s,double *%s,int %s", acc_name, loop_num,
              decl_args, sendbufname, send_counts_name);
      fprintf(headerfp, "int pack_%s_%d(%s,double *%s,int %s", acc_name,
              loop_num, decl_args, sendbufname, send_counts_name);
      if (options->variables_not_global && !options->data_dist) {
        fprintf(packfp, ", __DECLARATION_OF_%s", acc_name);
        fprintf(headerfp, ", __DECLARATION_OF_%s", acc_name);
      ...
      add_data_dist_parm_decl(headerfp, acc_name, prog);
      fprintf(headerfp, ");\n");
      generate_pack_or_unpack(packfp, prog, flow_out, flow_copy_text,
                              IN_FUNCTION, iters, src_copy_level, acc_nrows,
                              acc_name, send_counts_name);

      if (options->dynschedule) {
        sprintf(unpack_stmt_text + strlen(unpack_stmt_text),
                "%s = unpack_%s_%d(", currdisplsname, acc_name, loop_num);
        for (i = 0; i < src_copy_level; i++) {
          sprintf(unpack_stmt_text + strlen(unpack_stmt_text), "(int)%s[%d],",
                  recvbufname, i + 1);
        }
        sprintf(unpack_stmt_text + strlen(unpack_stmt_text), "%s,%s",
                recvbufname, currdisplsname);
      } else {
        sprintf(unpack_stmt_text + strlen(unpack_stmt_text),
                "%s[proc] = unpack_%s_%d(%s,%s,%s[proc]", currdisplsname,
                acc_name, loop_num, args, recvbufname, currdisplsname);
      }
      if (options->variables_not_global && !options->data_dist)
        sprintf(unpack_stmt_text + strlen(unpack_stmt_text), ",%s", acc_name);
      ...
      fprintf(packfp, "int unpack_%s_%d(%s,double *%s,int %s", acc_name,
              loop_num, decl_args, recvbufname, currdisplsname);
      fprintf(headerfp, "int unpack_%s_%d(%s,double *%s,int %s", acc_name,
              loop_num, decl_args, recvbufname, currdisplsname);
      if (options->variables_not_global && !options->data_dist) {
        fprintf(packfp, ", __DECLARATION_OF_%s", acc_name);
        fprintf(headerfp, ", __DECLARATION_OF_%s", acc_name);
      ...
      add_data_dist_parm_decl(headerfp, acc_name, prog);
      fprintf(headerfp, ");\n");
      generate_pack_or_unpack(packfp, prog, flow_out, flow_copyback_text,
                              IN_FUNCTION, iters, src_copy_level, acc_nrows,
                              acc_name, currdisplsname);
    }
  } else {
    for (l = 0; l < nloops; l++) {
      ...
      PlutoConstraints *receiver_tiles =
          get_receiver_tiles(wacc_stmts, num_accs, src_copy_level,
                             dest_copy_level, prog, l, pi_mappings);
      assert(receiver_tiles->ncols == total_copy_level + prog->npar + 1);
      assert(!pluto_constraints_is_empty(receiver_tiles));

      IF_DEBUG(printf("Data flow out intersection flow in set for %s and "
                      "dependent loop %d\n",
                      acc_name, l););
      IF_DEBUG(pluto_constraints_print(stdout, foifi));

      // === from here on we begin the code generation for each dependent
      // loop==========================================
      //
      char dest_args[1024];
      strcpy(dest_args, "");
      ...
      if (options->dynschedule) {
        sprintf(pack_stmt_text + strlen(pack_stmt_text),
                "pack_%s_%d_%d(%s,%s,%s, my_rank, nprocs, send_reqs_%s, "
                "index_%s, &current_index_%s",
                acc_name, loop_num, l, args, sendbufname, send_counts_name,
                acc_name, acc_name, acc_name);
      } else {
        sprintf(pack_stmt_text + strlen(pack_stmt_text),
                "pack_%s_%d_%d(%s,%s,%s, my_rank, nprocs", acc_name, loop_num,
                l, args, sendbufname, send_counts_name);
      }
      if (options->variables_not_global && !options->data_dist)
        sprintf(pack_stmt_text + strlen(pack_stmt_text), ",%s", acc_name);
      print_data_dist_parm_call(pack_stmt_text, arr);
      ...
      if (options->dynschedule) {
        sprintf(flow_cg_text + strlen(flow_cg_text), "\
                ...
                send_counts_name, acc_name, loop_num, l, passed_args, dest_args,
                sendbufname, send_counts_name);
      } else {
        sprintf(flow_cg_text + strlen(flow_cg_text), "\
      ...
      if (options->dynschedule) {
        fprintf(
            packfp, "void pack_%s_%d_%d(%s,std::vector<double *>&%s,int *%s, \
						int my_rank, int nprocs, std::vector<MPI_Request>& send_reqs",
            acc_name, loop_num, l, decl_args, sendbufname, send_counts_name);
        fprintf(
            headerfp, "void pack_%s_%d_%d(%s,std::vector<double *>&%s,int *%s, \
						int my_rank, int nprocs, std::vector<MPI_Request>& send_reqs",
            acc_name, loop_num, l, decl_args, sendbufname, send_counts_name);
        if (options->dynschedule) {
          fprintf(packfp, ",int *index,int *p_current_index");
        ...
        fprintf(packfp, "void pack_%s_%d_%d(%s,double **%s,int *%s, \
						int my_rank, int nprocs",
                acc_name, loop_num, l, decl_args, sendbufname,
                send_counts_name);
      ...
      generate_pack_or_unpack(packfp, prog, receiver_tiles, flow_cg_text,
                              IN_FUNCTION, dest_iters, src_copy_level,
                              dest_copy_level, acc_name, "");

      fprintf(packfp, "int pack_recv_%s_%d_%d(%s,%s,double *%s,int %s",
              acc_name, loop_num, l, decl_args, decl_dest_args, sendbufname,
              send_counts_name);
      ...
      add_data_dist_parm_decl(headerfp, acc_name, prog);
      fprintf(headerfp, ");\n");
      generate_pack_or_unpack(packfp, prog, foifi, flow_copy_text, IN_FUNCTION,
                              iters, total_copy_level, acc_nrows, acc_name,
                              send_counts_name);

      if (options->dynschedule) {
        sprintf(unpack_stmt_text + strlen(unpack_stmt_text),
                "%s = unpack_%s_%d_%d(", currdisplsname, acc_name, loop_num, l);
        for (i = 0; i < src_copy_level; i++) {
          sprintf(unpack_stmt_text + strlen(unpack_stmt_text), "(int)%s[%d],",
                  recvbufname, i + 1);
        }
        ...
        sprintf(unpack_stmt_text + strlen(unpack_stmt_text), "\
                ...
                currdisplsname, acc_name, loop_num, l, args, recvbufname,
                currdisplsname);
      }
      if (options->variables_not_global && !options->data_dist)
        sprintf(unpack_stmt_text + strlen(unpack_stmt_text), ",%s", acc_name);
      ...
      sprintf(flow_copyback_guard_text, "%s; if (recv_proc == my_rank) \
              ...
              pi_text, currdisplsname, acc_name, loop_num, l, passed_args,
              dest_args, recvbufname, currdisplsname);
      if (options->variables_not_global && !options->data_dist)
        sprintf(flow_copyback_guard_text + strlen(flow_copyback_guard_text),
                ",%s", acc_name);
      ...
      fprintf(
          packfp,
          "int unpack_%s_%d_%d(%s,double *%s,int %s, int my_rank, int nprocs",
          acc_name, loop_num, l, decl_args, recvbufname, currdisplsname);
      fprintf(
          headerfp,
          "int unpack_%s_%d_%d(%s,double *%s,int %s, int my_rank, int nprocs",
          acc_name, loop_num, l, decl_args, recvbufname, currdisplsname);
      ...
      fprintf(headerfp, ");\n");
      generate_pack_or_unpack(packfp, prog, receiver_tiles,
                              flow_copyback_guard_text, IN_FUNCTION, dest_iters,
                              src_copy_level, dest_copy_level, acc_name,
                              currdisplsname);

      fprintf(packfp, "int unpack_recv_%s_%d_%d(%s,%s,double *%s,int %s",
              acc_name, loop_num, l, decl_args, decl_dest_args, recvbufname,
              currdisplsname);
      fprintf(headerfp, "int unpack_recv_%s_%d_%d(%s,%s,double *%s,int %s",
              acc_name, loop_num, l, decl_args, decl_dest_args, recvbufname,
              currdisplsname);
      if (options->variables_not_global && !options->data_dist) {
        fprintf(packfp, ", __DECLARATION_OF_%s", acc_name);
      ...
      fprintf(headerfp, ");\n");
      generate_pack_or_unpack(packfp, prog, foifi, flow_copyback_text,
                              IN_FUNCTION, iters, total_copy_level, acc_nrows,
                              acc_name, currdisplsname);
...
Stmt **gen_comm_code_opt(int data_id, struct stmt_access_pair **wacc_stmts,
  ...
  char *flow_copy_text =
      malloc(strlen(sendbufname) + strlen("[") + strlen(send_count_name) +
             strlen("++] = ") + strlen(access) + 1);
  sprintf(flow_copy_text, "%s[%s++] = %s", sendbufname, send_count_name,
          access);
  ...
  sprintf(sigma_sender_text + strlen(sigma_sender_text),
          ", my_rank, nprocs, receiver_list)");
  ...
  if ((flow_out != NULL) && !pluto_constraints_is_empty(flow_out)) {
    ...
    if (options->dynschedule) {
      sprintf(flow_cg_text + strlen(flow_cg_text), "\
                ...
                index_%s = _i;",
              sendbufname, acc_name, acc_name, acc_name, acc_name, sendbufname,
              acc_name, sendbufname, acc_name, acc_name, acc_name);
      sprintf(flow_cg_text + strlen(flow_cg_text), "\
					%s = %d;",
              send_count_name,
              max_copy_level + 1); // one more for the loop number
      sprintf(flow_cg_text + strlen(flow_cg_text), "\
              ...
              send_count_name, acc_name, loop_num, args, sendbufname, acc_name,
              send_count_name);
    } else {
      sprintf(flow_cg_text + strlen(flow_cg_text), "\
					%s = pack_%s_%d(%s,%s,%s",
              send_count_name, acc_name, loop_num, args, sendbufname,
              send_count_name);
    }
    if (options->variables_not_global && !options->data_dist)
      sprintf(flow_cg_text + strlen(flow_cg_text), ",%s", acc_name);
    ...
    fprintf(packfp, "int pack_%s_%d(%s,double *%s,int %s", acc_name, loop_num,
            decl_args, sendbufname, send_count_name);
    fprintf(headerfp, "int pack_%s_%d(%s,double *%s,int %s", acc_name, loop_num,
            decl_args, sendbufname, send_count_name);
    if (options->variables_not_global && !options->data_dist) {
      fprintf(packfp, ", __DECLARATION_OF_%s", acc_name);
      fprintf(headerfp, ", __DECLARATION_OF_%s", acc_name);
    ...
    add_data_dist_parm_decl(headerfp, acc_name, prog);
    fprintf(headerfp, ");\n");
    generate_pack_or_unpack(packfp, prog, flow_out, flow_copy_text, IN_FUNCTION,
                            iters, src_copy_level, acc_nrows, acc_name,
                            send_count_name);
  }
  ...
  /* Create statement stub to get hold of the outer loops for the copy
   * stmt; rest of the loops to be added are the actual copy loops */
  Stmt *flow_copy_guard = create_helper_stmt(anchor_stmt, src_copy_level,
                                             flow_cg_text, COPY_OUT, loop_num);

  /* sigma, clear statements */
  char *clear_text =
      "for (__p=0; __p<nprocs; __p++) { receiver_list[__p] = 0; }";

  Stmt *send_recv_list_clear = create_helper_stmt(
      anchor_stmt, outer_dist_loop_level, clear_text, COMM_CALL, loop_num);
  Stmt *sigma_stmt = create_helper_stmt(anchor_stmt, src_copy_level,
                                        sigma_sender_text, SIGMA, loop_num);
  ...
  if (options->dynschedule) {
    ...
    sprintf(comm_text + strlen(comm_text), "\
				for (__p=0; __p<nprocs; __p++) {\
				if(receiver_list[__p] != 0) {");
    sprintf(comm_text + strlen(comm_text), "%s[index_%s][0] = %d;", sendbufname,
            acc_name, loop_num);
    for (i = 0; i < src_copy_level; i++) {
      sprintf(comm_text + strlen(comm_text), "%s[index_%s][%d] = t%d;",
              sendbufname, acc_name, i + 1, i + 1);
    }
    sprintf(comm_text + strlen(comm_text), "\
		        IF_DYNSCHEDULER_DEBUG_PRINT(\
    ...
    if (options->timereport)
      sprintf(comm_text + strlen(comm_text), "IF_TIME(__total_count += %s);",
              send_count_name);
    sprintf(comm_text + strlen(comm_text), "\
            ...
            sendbufname, acc_name, send_count_name, data_tag, acc_name,
            acc_name, acc_name, acc_name);
  } else {
    if (options->timereport)
      sprintf(comm_text, "IF_TIME(t_comm_start = rtclock());");
    ...
    sprintf(comm_text + strlen(comm_text), "\
				...
				MPI_Alltoall(%s, 1, MPI_INT,\
					%s, 1, MPI_INT, MPI_COMM_WORLD);",
            send_counts_name, send_count_name, send_counts_name,
            recv_counts_name);
    sprintf(comm_text + strlen(comm_text), "\
				req_count=0;\
				for (__p=0; __p<nprocs; __p++) {\
...
recv_buf, max_recv_count, MPI_DOUBLE, MPI_COMM_WORLD)");*/
  ...
  if (options->dynschedule) {
    ...
    sprintf(prog->decls + strlen(prog->decls),
            "%s = (double *) polyrt_max_alloc(%s, nprocs*send_buf_size_%s, "
            "&recv_buf_size_%s);\n",
            recvbufname, recvbufname, acc_name, acc_name);
    sprintf(prog->decls + strlen(prog->decls),
            "\tfor (__p=0; __p<nprocs; __p++) {\
  ...
  char *flow_copyback_text = malloc(
      strlen(access) + strlen(" = ") + strlen(recvbufname) + strlen("[") +
      strlen(displsname) + strlen("[proc]+ count++];") + 1);
  sprintf(flow_copyback_text, "%s = %s[%s + count++];", access, recvbufname,
          displsname);
  ...
  if ((flow_out != NULL) && !pluto_constraints_is_empty(flow_out)) {
    if (options->dynschedule) {
      // one more for the loop number
      sprintf(proc_stmt_text + strlen(proc_stmt_text), "%s = %d; \
					%s = unpack_%s_%d(",
              displsname, max_copy_level + 1, displsname, acc_name, loop_num);
      for (i = 0; i < src_copy_level; i++) {
        sprintf(proc_stmt_text + strlen(proc_stmt_text), "(int)%s[%d],",
                recvbufname, i + 1);
      }
      sprintf(proc_stmt_text + strlen(proc_stmt_text), "%s,0,%s", recvbufname,
              displsname);
      if (options->variables_not_global && !options->data_dist)
        sprintf(proc_stmt_text + strlen(proc_stmt_text), ",%s", acc_name);
      if (options->data_dist)
        print_data_dist_parm_call(proc_stmt_text, arr);
      sprintf(proc_stmt_text + strlen(proc_stmt_text), ");");
    } else {
      sprintf(proc_stmt_text + strlen(proc_stmt_text),
              "proc = pi_%d(%s%s, nprocs); ", loop_num, args, params);
      sprintf(proc_stmt_text + strlen(proc_stmt_text),
              "if ((proc != my_rank) && (%s[proc] > 0)) { \
					for (__p=0; __p<nprocs; __p++) { receiver_list[__p] = 0; } \
					%s, proc, nprocs, receiver_list); ",
              recv_counts_name, sigma_text);
      sprintf(proc_stmt_text + strlen(proc_stmt_text),
              "for (__p=0; __p<nprocs; __p++) { if (receiver_list[__p] != 0) \
					{ curr_%s[proc] = unpack_%s_%d(%s,%s,%s[proc],curr_%s[proc]",
              displsname, acc_name, loop_num, args, recvbufname, displsname,
              displsname);
      if (options->variables_not_global && !options->data_dist)
        sprintf(proc_stmt_text + strlen(proc_stmt_text), ",%s", acc_name);
      print_data_dist_parm_call(proc_stmt_text, arr);
    ...
    fprintf(packfp, "int unpack_%s_%d(%s,double *%s,int %s,int count", acc_name,
            loop_num, decl_args, recvbufname, displsname);
    fprintf(headerfp, "int unpack_%s_%d(%s,double *%s,int %s,int count",
            acc_name, loop_num, decl_args, recvbufname, displsname);
    if (options->variables_not_global && !options->data_dist) {
      fprintf(packfp, ", __DECLARATION_OF_%s", acc_name);
      fprintf(headerfp, ", __DECLARATION_OF_%s", acc_name);
    }
  ...
  /* Create statement stub to get hold of the outer loops for the copy
   * stmt; rest of the loops to be added are the actual copy loops */
  Stmt *copyback_proc_stmt = create_helper_stmt(
      anchor_stmt, src_copy_level, proc_stmt_text, COPY_IN, loop_num);
...
void pluto_dynschedule_common_codegen(PlutoProg *prog, FILE *sigmafp,
  ...
  for (l = 0; l < nloops; l++) {
    Ploop *loop = loops[l];
    if (options->data_dist) {
      // due to my_rank as a parameter, data tile domains doesnt assume my_rank
      // parameter
      if (options->distmem)
        prog->npar--;

      gen_data_tile_alloc_cloog_code(prog, l, loop->stmts, loop->nstmts,
                                     copy_level[l], sigmafp, headerfp);
      gen_data_tile_ref_count_update_cloog_code(
          prog, l, loop->stmts, loop->nstmts, copy_level[l], sigmafp, headerfp);
  ...
  if (options->data_dist && !options->distmem) {
    ...
    pluto_dist_generate_copy_back_code(loops, nloops, copy_level, headerfp,
                                       prog);
  }
  if (options->dynschedule_graph) {
    gen_dynschedule_graph_main_text_code(prog, loops, nloops, copy_level,
                                         outfp);
  } else {
...
int pluto_dynschedule_graph_codegen(PlutoProg *prog, FILE *sigmafp, FILE *outfp,
  ...
  fprintf(
      outfp,
      "tbb::flow::broadcast_node<tbb::flow::continue_msg> __start(__graph);\n");
  ...
  if (options->timereport) {
    ...
    fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"DAG creation time: "
                   "%%0.6lf s\\n\", t_create_dag);\n\n");
    fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Total time: %%0.6lf "
                   "s\\n\", t_local);\n\n");
...
int pluto_dynschedule_codegen(PlutoProg *prog, FILE *sigmafp, FILE *outfp,
  ...
  if (options->timereport) {
    fprintf(
        outfp,
        "\tint waiting_pops_start[_num_threads], waiting_pops[_num_threads], \
                waiting_pops_threads;\n");
    fprintf(outfp, "\tfor (__tid=0; __tid<_num_threads; __tid++) "
                   "waiting_pops[__tid] = 0;\n");
    fprintf(outfp, "\tdouble t_local;\n");
    fprintf(outfp, "\tdouble t_tasks_create;\n");
    fprintf(
        outfp,
        "\tdouble t_tasks_manage_start[_num_threads], t_tasks_manage[_num_threads], \
                t_tasks_manage_threads;\n");
    fprintf(outfp, "\tfor (__tid=0; __tid<_num_threads; __tid++) "
                   "t_tasks_manage[__tid] = 0;\n");
    fprintf(outfp, "\tdouble t_comp_start[_num_threads], t_comp[_num_threads], "
                   "t_comp_threads_min, t_comp_threads;\n");
    fprintf(outfp, "\tdouble t_comp_mean, t_comp_sd;\n");
    fprintf(
        outfp,
        "\tfor (__tid=0; __tid<_num_threads; __tid++) t_comp[__tid] = 0;\n");
    fprintf(outfp, "\tdouble t_wait_comp_start[_num_threads], "
                   "t_wait_comp[_num_threads], t_wait_comp_threads;\n");
    fprintf(outfp, "\tfor (__tid=0; __tid<_num_threads; __tid++) "
                   "t_wait_comp[__tid] = 0;\n");

    if (options->data_dist) {
      fprintf(outfp, "\tdouble t_data_mang_start[_num_threads], "
                     "t_data_mang[_num_threads], t_data_mang_threads;\n");
      fprintf(outfp, "\tfor (__tid=0; __tid<_num_threads; __tid++) "
                     "t_data_mang[__tid] = 0;\n");
    }
  ...
  if (options->timereport) {
    ...
    fprintf(outfp, "\tchar *buffer = (char *)malloc(4096 * sizeof(char));\n");
    fprintf(outfp, "\tstrcpy(buffer, \"\");\n");
    fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Minimum computation "
                   "time across all threads: %%0.6lf s\\n\", "
                   "t_comp_threads_min);\n");
    fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Maximum computation "
                   "time across all threads: %%0.6lf s\\n\", "
                   "t_comp_threads);\n");
    fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Mean computation time "
                   "across all threads: %%0.6lf s\\n\", t_comp_mean);\n");
    fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Standard deviation in "
                   "computation time across all threads: %%0.6lf s\\n\", "
                   "t_comp_sd);\n");
    fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Load imbalance factor "
                   "(standard-deviation/mean): %%0.2lf %%%%\\n\", "
                   "100*t_comp_sd/t_comp_mean);\n");
    fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Maximum 'waiting for "
                   "computation' time across all threads: %%0.6lf s\\n\", "
                   "t_wait_comp_threads);\n");
    fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Maximum number of "
                   "waiting/unsuccessful POPs across all threads: %%d\\n\", "
                   "waiting_pops_threads);\n");
    fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Maximum tasks "
                   "management time across all threads: %%0.6lf s\\n\", "
                   "t_tasks_manage_threads);\n");
    fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Tasks creation time: "
                   "%%0.6lf s\\n\", t_tasks_create);\n");
    if (options->data_dist) {
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Data init time: "
                     "%%0.6lf s\\n\", t_data_init);\n");
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Data mang time: "
                     "%%0.6lf s\\n\", t_data_mang_threads);\n");
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Copy back time: "
                     "%%0.6lf s\\n\", t_writeout);\n");
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Sum of split times: "
                     "%%0.6lf s\\n\", "
                     "t_comp_threads+t_wait_comp_threads+t_tasks_manage_"
                     "threads+t_tasks_create+t_data_init+t_data_mang_threads+t_"
                     "writeout);\n");
    } else {
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Sum of split times: "
                     "%%0.6lf s\\n\", "
                     "t_comp_threads+t_wait_comp_threads+t_tasks_manage_"
                     "threads+t_tasks_create);\n");
    }
    fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Total time: %%0.6lf "
                   "s\\n\", t_local);\n");
    fprintf(outfp, "\tfprintf(stdout, \"%%s\\n\", buffer);\n");
...
int pluto_distmem_codegen(PlutoProg *prog, FILE *cloogfp, FILE *sigmafp,
  ...
  if (options->data_dist) {
    fprintf(outfp, "\tdouble t_data_init_start, t_data_init= 0.0, "
                   "t_globaldata_init= 0.0;\n");
    if (!options->dynschedule)
      fprintf(outfp, "\tdouble t_data_mang_start, t_data_mang= 0.0, "
                     "t_globaldata_mang= 0.0;\n");
  }

  fprintf(outfp, "\n##ifndef GLOBAL_MY_RANK\n\tint my_rank;\n##endif\n");
  fprintf(outfp, "\n##ifndef __MAX_NUM_RECVS\n\t##define __MAX_NUM_RECVS "
                 "(nprocs*_num_threads*16)\n##endif\n");
  fprintf(outfp, "\n##ifndef __MAX_NUM_SENDS\n\t##define __MAX_NUM_SENDS "
                 "(nprocs*_num_threads*8)\n##endif\n");
  ...
  if (options->dynschedule) {
    ...
    fprintf(outfp, "\tMPI_Init_thread(NULL, NULL, MPI_THREAD_MULTIPLE, "
                   "&thread_support_provided);\n");
    fprintf(outfp,
            "\tassert(thread_support_provided == MPI_THREAD_MULTIPLE);\n");
  } else {
  ...
  if (options->timereport) {
    if (options->dynschedule) {
      ...
      fprintf(outfp, "\tfor (__tid=0; __tid<_num_threads; __tid++) "
                     "t_tasks_manage[__tid] = 0;\n");
      fprintf(outfp,
              "\tdouble t_comp_start[_num_threads], t_comp[_num_threads], \
                    t_comp_threads_min, t_globalcomp_min, t_comp_threads, t_globalcomp;\n");
      fprintf(outfp, "\tdouble t_comp_mean, t_comp_sd, t_globalcomp_mean, "
                     "t_globalcomp_sd;\n");
      fprintf(
          outfp,
          "\tfor (__tid=0; __tid<_num_threads; __tid++) t_comp[__tid] = 0;\n");
      ...
      fprintf(outfp, "\tdouble t_pack_start[_num_threads], "
                     "t_pack[_num_threads], t_pack_threads, t_globalpack;\n");
      fprintf(
          outfp,
          "\tfor (__tid=0; __tid<_num_threads; __tid++) t_pack[__tid] = 0;\n");
    } else {
      fprintf(outfp,
    ...
    if (options->dynschedule) {
      fprintf(outfp, "\tdouble t_wait_unpack_start, t_wait_unpack = 0.0, "
                     "t_globalwait_unpack = 0.0;\n");
    }
    fprintf(outfp, "\tdouble t_local = 0.0, t_global = 0.0;\n");
    if (options->dynschedule) {
      fprintf(outfp, "\tdouble __total_count[_num_threads];\n");
      fprintf(outfp, "\tfor (__tid=0; __tid<_num_threads; __tid++) "
                     "__total_count[__tid] = 0;\n");
  ...
  if (options->timereport) {
    ...
    if (options->dynschedule) {
      fprintf(outfp, "\tfor (__tid=0; __tid<_num_threads; __tid++) "
                     "__total_count_threads += __total_count[__tid];\n");
      fprintf(outfp, "\twaiting_pops_threads = waiting_pops[0];\n\
                    for (__tid=1; __tid<_num_threads; __tid++) if (waiting_pops_threads < waiting_pops[__tid]) \
                    waiting_pops_threads = waiting_pops[__tid];\n");
      fprintf(outfp, "\tt_tasks_manage_threads = t_tasks_manage[0];\n\
                    ...
                    t_tasks_manage_threads = t_tasks_manage[__tid];\n");
      fprintf(outfp,
              "\tif (__tid_receiver != 0) t_comp_threads_min = t_comp[0];\n\
    ...
    if (options->verify_output)
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Write-out time: "
                     "%%0.6lf s\\n\", t_writeout);\n\n");
    if (options->dynschedule) {
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Values communicated: "
    ...
    if (options->dynschedule) {
      ...
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Load imbalance factor "
                     "(standard-deviation/mean): %%0.2lf %%%%\\n\", "
                     "100*t_comp_sd/t_comp_mean);\n");
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"'Waiting for "
                     "computation' time: %%0.6lf s\\n\", "
                     "t_wait_comp_threads);\n");
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Tasks creation time: "
                     "%%0.6lf s\\n\", t_tasks_create);\n");
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Tasks management "
                     "time: %%0.6lf s\\n\", t_tasks_manage_threads);\n");
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Packing time: %%0.6lf "
                     "s\\n\", t_pack_threads);\n");
    } else {
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Computation time: "
                     "%%0.6lf s\\n\", t_comp);\n");
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Communication time: "
                     "%%0.6lf s\\n\", t_comm);\n");
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Packing time: %%0.6lf "
                     "s\\n\", t_pack);\n");
    }
    fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Unpacking time: %%0.6lf "
                   "s\\n\", t_unpack);\n");
    if (options->dynschedule) {
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"'Waiting for "
                     "unpacking' time: %%0.6lf s\\n\", t_wait_unpack);\n");
      // unpacking time isn't considered since ideally, it should be overlapped
      // with the rest
      if (options->data_dist) {
        fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Sum of split times: "
                       "%%0.6lf s\\n\", "
                       "t_comp_threads + t_wait_comp_threads + t_tasks_create "
                       "+ t_tasks_manage_threads + t_pack_threads + "
                       "t_data_mang_threads);\n\n");
      } else {
        fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Sum of split times: "
                       "%%0.6lf s\\n\", "
      ...
      if (options->data_dist) {
        ...
        fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Sum of split times: "
                       "%%0.6lf s\\n\", t_comp + t_comm + t_pack + "
                       "t_unpack);\n\n");
    }
    if (options->data_dist) {
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Total time - "
                     "write-out - data_init time: %%0.6lf s\\n\", "
                     "t_local);\n\n");
    } else {
      fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"Total time minus "
                     "write-out time: %%0.6lf s\\n\", t_local);\n");
    }
    fprintf(outfp, "\tsprintf(buffer+strlen(buffer), \"-------\");\n");
    fprintf(outfp, "\tfprintf(stdout, \"%%s\\n\", buffer);\n");
    fprintf(outfp, "\t}\n");
    if (options->dynschedule) {
      fprintf(outfp, "\tMPI_Reduce(&__total_count_threads, &__total_count_all, "
                     "1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);\n");
    } else {
      fprintf(outfp, "\tMPI_Reduce(&__total_count, &__total_count_all, 1, "
                     "MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);\n");
    }
    if (options->dynschedule) {
      fprintf(outfp, "\tMPI_Allreduce(MPI_IN_PLACE, &t_globalcomp_mean, 1, "
                     "MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);\n");
      fprintf(outfp, "\tt_globalcomp_mean = "
                     "t_globalcomp_mean/((_num_threads-1)*nprocs);\n");
      ...
      if (options->data_dist) {
        fprintf(outfp, "\tMPI_Reduce(&t_data_init, &t_globaldata_init, 1, "
                       "MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);\n");
        fprintf(outfp, "\tMPI_Reduce(&t_data_mang_threads, &t_globaldata_mang, "
                       "1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);\n");
      }
      fprintf(outfp, "\tMPI_Reduce(&waiting_pops_threads, "
                     "&waiting_pops_global, 1, MPI_INT, MPI_MAX, 0, "
                     "MPI_COMM_WORLD);\n");
      fprintf(outfp, "\tMPI_Reduce(&t_comp_threads_min, &t_globalcomp_min, 1, "
                     "MPI_DOUBLE, MPI_MIN, 0, MPI_COMM_WORLD);\n");
      fprintf(outfp, "\tMPI_Reduce(&t_comp_threads, &t_globalcomp, 1, "
                     "MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);\n");
      fprintf(outfp, "\tMPI_Reduce(&t_wait_comp_threads, &t_globalwait_comp, "
                     "1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);\n");
      fprintf(outfp, "\tMPI_Reduce(&t_tasks_create, &t_globaltasks_create, 1, "
                     "MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);\n");
      fprintf(outfp, "\tMPI_Reduce(&t_tasks_manage_threads, "
                     "&t_globaltasks_manage, 1, MPI_DOUBLE, MPI_MAX, 0, "
                     "MPI_COMM_WORLD);\n");
      fprintf(outfp, "\tMPI_Reduce(&t_pack_threads, &t_globalpack, 1, "
                     "MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);\n");
    } else {
      fprintf(outfp, "\tMPI_Allreduce(&t_comp, &t_comp_mean, 1, MPI_DOUBLE, "
                     "MPI_SUM, MPI_COMM_WORLD);\n");
      fprintf(outfp, "\tt_comp_mean = t_comp_mean/nprocs;\n");
      fprintf(outfp,
              "\tt_comp_sd = (t_comp-t_comp_mean)*(t_comp-t_comp_mean);\n");
      fprintf(outfp, "\tMPI_Allreduce(MPI_IN_PLACE, &t_comp_sd, 1, MPI_DOUBLE, "
                     "MPI_SUM, MPI_COMM_WORLD);\n");
      fprintf(outfp, "\tt_comp_sd = sqrt(t_comp_sd/nprocs);\n");
      if (options->data_dist) {
        fprintf(outfp, "\tMPI_Reduce(&t_data_init, &t_globaldata_init, 1, "
                       "MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);\n");
        fprintf(outfp, "\tMPI_Reduce(&t_data_mang, &t_globaldata_mang, 1, "
                       "MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);\n");
      }
      fprintf(outfp, "\tMPI_Reduce(&t_comp, &t_globalcomp, 1, MPI_DOUBLE, "
                     "MPI_MAX, 0, MPI_COMM_WORLD);\n");
      fprintf(outfp, "\tMPI_Reduce(&t_comm, &t_globalcomm, 1, MPI_DOUBLE, "
                     "MPI_MAX, 0, MPI_COMM_WORLD);\n");
      fprintf(outfp, "\tMPI_Reduce(&t_pack, &t_globalpack, 1, MPI_DOUBLE, "
                     "MPI_MAX, 0, MPI_COMM_WORLD);\n");
    }
    fprintf(outfp, "\tMPI_Reduce(&t_unpack, &t_globalunpack, 1, MPI_DOUBLE, "
                   "MPI_MAX, 0, MPI_COMM_WORLD);\n");
    if (options->dynschedule) {
      fprintf(outfp, "\tMPI_Reduce(&t_wait_unpack, &t_globalwait_unpack, 1, "
                     "MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);\n");
    }
    fprintf(outfp, "\tMPI_Reduce(&t_local, &t_global, 1, MPI_DOUBLE, MPI_MAX, "
                   "0, MPI_COMM_WORLD);\n");
    fprintf(outfp, "\tif (my_rank==0) {\n");
    fprintf(outfp, "\t\tstrcpy(buffer, \"SUMMARY\\n\");\n");
    if (options->verify_output)
      fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Write-out time "
                     "spent in master node: %%0.6lf s\\n\", t_writeout);\n");
    if (options->data_dist) {
      fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Maximum data init "
                     "time spent across all nodes: %%0.6lf s\\n\", "
                     "t_globaldata_init);\n\n");
      fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Maximum data mang "
                     "time spent across all nodes: %%0.6lf s\\n\", "
                     "t_globaldata_mang);\n\n");
    }
    fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Total communication "
                   "volume across all nodes: %%0.6lf GB\\n\", "
    ...
    if (options->dynschedule) {
      fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Minimum computation "
                     "time spent across all nodes: %%0.6lf s\\n\", "
                     "t_globalcomp_min);\n");
      fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Mean computation "
                     "time across all nodes: %%0.6lf s\\n\", "
                     "t_globalcomp_mean);\n");
      fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Standard deviation "
                     "in computation time across all nodes: %%0.6lf s\\n\", "
                     "t_globalcomp_sd);\n");
      fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Load imbalance "
                     "factor (standard-deviation/mean): %%0.2lf %%%%\\n\", "
                     "100*t_globalcomp_sd/t_globalcomp_mean);\n");
      fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Maximum 'waiting "
                     "for computation' time spent across all nodes: %%0.6lf "
                     "s\\n\", t_globalwait_comp);\n");
      fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Maximum number of "
                     "waiting/unsuccessful POPs across all nodes: %%d\\n\", "
                     "waiting_pops_global);\n");
      fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Maximum tasks "
                     "creation time spent across all nodes: %%0.6lf s\\n\", "
                     "t_globaltasks_create);\n");
      fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Maximum tasks "
                     "management time spent across all nodes: %%0.6lf s\\n\", "
                     "t_globaltasks_manage);\n");
      ;
    } else {
      fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Mean computation "
                     "time across all nodes (not per-thread): %%0.6lf s\\n\", "
                     "t_comp_mean);\n");
      fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Standard deviation "
                     "in computation time across all nodes (not per-thread): "
                     "%%0.6lf s\\n\", t_comp_sd);\n");
      fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Load imbalance "
                     "factor (standard-deviation/mean): %%0.2lf %%%%\\n\", "
                     "100*t_comp_sd/t_comp_mean);\n");
      fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Maximum "
                     "communication time spent across all nodes: %%0.6lf "
                     "s\\n\", t_globalcomm);\n");
    }
    fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Maximum packing time "
                   "spent across all nodes: %%0.6lf s\\n\", t_globalpack);\n");
    fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Maximum unpacking "
                   "time spent across all nodes: %%0.6lf s\\n\", "
                   "t_globalunpack);\n");
    if (options->dynschedule) {
      fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Maximum 'waiting "
                     "for unpacking' time spent across all nodes: %%0.6lf "
                     "s\\n\", t_globalwait_unpack);\n");
    }
    fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"Maximum total time "
                   "spent across all nodes: %%0.6lf s\\n\", t_global);\n");
    fprintf(outfp, "\t\tsprintf(buffer+strlen(buffer), \"-------\");\n");
    fprintf(outfp, "\t\tfprintf(stdout, \"%%s\\n\", buffer);\n");
    fprintf(outfp, "\t}\n");
...
void pluto_add_distmem_decls(PlutoProg *prog, FILE *headerfp) {
  ...
  for (i = 0; i < num; i++) {
    ...
    /* lw count name */
    sprintf(prog->decls + strlen(prog->decls), "int lw_count_%s = 0;\n", name);
    if (options->dynschedule) {
      sprintf(prog->decls + strlen(prog->decls),
              "int send_counts_%s[__tid][nprocs];\n", name);
      sprintf(prog->decls + strlen(prog->decls), "max_num_elements_%s = 0;\n",
              name);
    ...
    sprintf(prog->decls + strlen(prog->decls), "int recv_counts_%s[nprocs];\n",
            name);
    sprintf(prog->decls + strlen(prog->decls),
            "int lw_recv_counts_%s[nprocs];\n", name);
    sprintf(prog->decls + strlen(prog->decls), "int displs_%s[nprocs];\n",
            name);
    sprintf(prog->decls + strlen(prog->decls), "int displs_lw_%s[nprocs];\n",
            name);
    if (options->commopt_foifi || options->commopt_fop) {
      if (options->dynschedule) {
        sprintf(prog->decls + strlen(prog->decls),
      ...
      if (options->dynschedule) {
        ...
        sprintf(prog->decls + strlen(prog->decls), "int send_count_%s = 0;\n",
                name);
        sprintf(prog->decls + strlen(prog->decls),
                "int curr_displs_%s[nprocs];\n", name);
        sprintf(prog->decls + strlen(prog->decls),
    ...
    sprintf(prog->decls + strlen(prog->decls), "double *lw_buf_%s = NULL;\n",
            name);
    sprintf(prog->decls + strlen(prog->decls), "size_t lw_buf_size_%s = 0;\n",
            name);
    sprintf(prog->decls + strlen(prog->decls),
...
void pluto_dist_initalize_array_domains(PlutoProg *prog, int nloops) {
  ...
  for (i = 0; i < prog->narrays; ++i) {
    ...
    arr->parmetric_domain =
        (PlutoConstraints **)malloc(nloops * sizeof(PlutoConstraints *));
    for (j = 0; j < nloops; j++) {
...
void pluto_dynschedule_common_parallelize(PlutoProg *prog, FILE *sigmafp,
  ...
  for (l = 0; l < nloops; l++) {
    sprintf(tasks_loops_decl + strlen(tasks_loops_decl), ",%stasks_loop%d",
            type, l);
  }
  ...
  for (l = 0; l < nloops; l++) {
    ...
    wacc_stmts = get_write_access_with_stmts(loop->stmts, loop->nstmts,
                                             &num_data[l], &num_stmts_per_wacc);

    tasks_stmts[l] = gen_tasks_code(loop->stmts, loop->nstmts, copy_level, prog,
                                    num_data[l], l, nloops, pi_mappings,
                                    tasks_loops_decl, sigmafp, headerfp);
  ...
  gen_init_tasks_cloog_code(prog, tasks_stmts, nloops, tasks_loops_decl,
                            headerfp);
...
#define min(x,y)    ((x) < (y)? (x) : (y))\n\n");
  ...
  pluto_dynschedule_common_parallelize(prog, sigmafp, headerfp, loops, nloops,
                                       pi_mappings, copy_level);
...
#define min(x,y)    ((x) < (y)? (x) : (y))\n\n");
    ...
    if (options->compute_pi) {
      pluto_dist_gen_compute_pi(nloops, loops, copy_level, prog, pi_mappings,
                                pifp, headerfp);
    }
...
char *get_add_vertex_weight_stmt_text(int phase_level, int **is_scalar_dim,
  ...
  sprintf(text + strlen(text), ";");
  sprintf(text + strlen(text), "for(i=0;i<total_num_phases;i++) "
                               "{vertex_wgt[vertex_count++] = "
                               "(i==cur_phase)?1:0;}");
...
Stmt **gen_vertex_wgt_stmt_code(struct stmt_access_pair **acc_stmts,
  ...
  char *stmt_text =
      get_add_vertex_weight_stmt_text(phase_level, is_scalar_dim, loop_num);

  char *gaurd_stmt_text = malloc(1024);
  sprintf(gaurd_stmt_text, "prev_phase = cur_phase + 1;");

  Stmt *vertex_wights_stmt = create_helper_stmt(
      anchor_stmt, src_copy_level, stmt_text, IN_FUNCTION, loop_num);

  Stmt *gaurd_stmt = create_helper_stmt(anchor_stmt, 0, gaurd_stmt_text,
                                        IN_FUNCTION, loop_num);
...
char *get_add_edge_weight_stmt_text() {
  ...
  sprintf(text, "for(i=0;i<total_num_phases;i++) {vetex_wgt[count++] = "
                "(i==cur_phase)?1:0}");
...
Stmt **gen_edge_wgt_stmt_code(struct stmt_access_pair ***acc_stmts,
  ...
  sprintf(text + strlen(text), "update_edges(cur_vertex, edges, &num_edges, "
                               "edge_wgts, vadj, &num_vadj, comm_cost); ");
  Stmt *edge_wights_stmt = create_helper_stmt(anchor_stmt, src_copy_level, text,
                                              IN_FUNCTION, loop_num);
...
void gen_comm_vol_code(struct stmt_access_pair **wacc_stmts, int num_accs,
  ...
  for (k = 0; k < num_accs; k++) {
    ...
    for (i = 0; i < prog->ndeps; i++) {
      ...
      /* If the dependence doesn't originate from this access */
      //              if (dep->src_acc != wacc_stmts[k]->acc && dep->dest_acc !=
      // wacc_stmts[k]->acc) continue;
      if (dep->src_acc != wacc_stmts[k]->acc)
        continue;
  ...
  for (l = 0; l < nloops; l++) {
    ...
    for (j = 0; j < acc_nrows; j++) {
      pluto_constraints_interchange_cols(foifi_sets[l], j,
                                         j + total_copy_level);
    }
    for (j = 0; j < acc_nrows; j++) {
    ...
    PlutoMatrix *trans =
        get_data_tile_trans_func(total_copy_level, acc_nrows, prog);
    ...
    fprintf(outfp, "printf(\" comm vol of %s_%d_%d  from ", acc_name, loop_num,
            l);
    for (i = 0; i < src_copy_level; i++) {
      fprintf(outfp, "%%d ");
    }
...
void gen_edge_wgt_func_code(struct stmt_access_pair **wacc_stmts, int naccs,
  ...
  generate_sigma_common(wacc_stmts, naccs, copy_level, prog, loop_num,
                        pi_mappings, 1, &broadcast, &num_dest_loops,
                        tdpoly_list, dep_loop_num_list, trans, iters, indices);
  ...
  for (i = 0; i < num_dest_loops; i++) {
    ...
    for (j = 0; j < dest_copy_level; j++) {
      ...
      if (is_scalar_dim[dep_loop_num][j])
        continue;
    ...
    sprintf(sigma_text + strlen(sigma_text), "+= comm_vol_%s_%d_%d(", acc_name,
            loop_num, dep_loop_num);
    for (j = 0; j < src_copy_level; j++) {
...
void fprint_partition_func(FILE *outfp) {
  ...
  fprintf(outfp,
          "void partition_print(long num_nodes, long *partition, long obj_val, "
          "long ret){"
          "\n int i = 0; \n"
          "remove(\"partition\");\n"
...
void fprint_debug_graph_func(FILE *outfp) {
  ...
  fprintf(outfp, " long *edges, long num_edges, long  *edge_wgts, long "
                 "*vertex_wgt, long num_vertex_wgt, long total_num_phases, "
  ...
  fprintf(outfp, "fprintf(fp,\"#vertices = %%d\\n vertex weights\\n\", "
                 "num_vertex_wgt);\n");
  fprintf(outfp, " \n for(i = 0; i< num_vertex_wgt; i++) { "
                 "if((i%%total_num_phases) == 0) fprintf(fp, \"\\n\"); \n "
...
void fprint_update_edges_func(FILE *outfp, int max_copy_level, int nloops) {
  ...
  fprintf(outfp, "long cur_vertex, long *edges, long  *num_edges, long "
                 "*edge_wgts, long  *vadj, long *num_vadj, long comm_cost[%d]",
          nloops);
...
int pluto_dist_gen_compute_pi(int nloops, Ploop **loops, int *copy_level,
  ...
  int *non_scalar_copy_level =
      get_num_non_scalar_dims(nloops, loops, copy_level, is_scalar_dim);
  ...
  for (l = 0; l < nloops; l++) {
    ...
    rwacc_stmts = get_read_write_access_with_stmts(loop->stmts, loop->nstmts,
                                                   &num_read_write_data[l],
                                                   &num_stmts_per_rwacc);
    ...
    vertex_wgt_stmts[l][0] = gen_vertex_wgt_stmt_code(
        wacc_stmts[0], num_stmts_per_wacc[0], prog, copy_level,
        non_scalar_copy_level, is_scalar_dim, l);

    for (i = 0; i < num_read_write_data[l]; i++) {

      gen_comm_vol_code(rwacc_stmts[i], num_stmts_per_rwacc[i], nloops, prog,
                        anchor_stmt, copy_level, is_scalar_dim, l, pi_mappings,
                        headerfp, outfp);

      gen_edge_wgt_func_code(rwacc_stmts[i], num_stmts_per_rwacc[i], copy_level,
                             non_scalar_copy_level, is_scalar_dim,
                             max_copy_level, prog, l, nloops, pi_mappings,
                             outfp, headerfp);
    }

    edge_wgt_stmts[l][0] = gen_edge_wgt_stmt_code(
        rwacc_stmts, num_read_write_data[l], anchor_stmt, prog, copy_level,
        non_scalar_copy_level[l], is_scalar_dim, max_copy_level, l);
  ...
  fprintf(outfp, "long i, total_num_phases = %s, vertex_count = 0, cur_phase = "
                 "0, prev_phase = 0;\n",
          get_total_num_phases(nloops, loops, copy_level, non_scalar_copy_level,
                               is_scalar_dim, prog));

  fprintf(outfp, "long vertex_wgt[total_vertices * total_num_phases];\n");
  fprintf(outfp, "long partitions[total_vertices], objval = 0, ret = 0, parts "
                 "= nprocs;\n");
  ...
  fprintf(outfp, "debug_graph_print(edges, num_edges, edge_wgts, vertex_wgt, "
                 "vertex_count, "
                 "total_num_phases, vadj, num_vadj);\n");

  fprintf(outfp, "ret = METIS_PartGraphKway(&total_vertices, "
                 "&total_num_phases, vadj, edges,"
                 "vertex_wgt, NULL, edge_wgts, &parts, NULL, NULL, NULL, "
                 "&objval, partitions);\n ");
...
int pluto_dist_gen_intial_code(int nloops, Ploop **loops, int *copy_level,
  ...
  for (l = 0; l < nloops; l++) {
    ...
    ref_count_initialize_stmts[l] =
        (Stmt ***)malloc(num_read_write_data[l] * sizeof(Stmt **));
    ref_count_writeout_stmts[l] =
        (Stmt ***)malloc(num_write_data[l] * sizeof(Stmt **));

    for (i = 0; i < num_read_write_data[l]; ++i) {
      pluto_dist_update_arr_domain(acc_stmts[i], num_stmts_per_acc[i], prog,
                                   copy_level, l);
    }
    ...
    for (i = 0; i < num_read_data[l]; ++i) {
      PlutoConstraints *read_in = get_read_in_constraints(
          racc_stmts[i], num_stmts_per_racc[i], copy_level[l], l, prog);

      gen_read_in_data_tile_alloc_cloog_code(racc_stmts[i],
                                             num_stmts_per_racc[i], read_in,
                                             prog, l, copy_level[l], headerfp);

      gen_read_in_copy_cloog_code(racc_stmts[i], num_stmts_per_racc[i], read_in,
                                  prog, l, copy_level[l], headerfp);

      read_in_stmts[l][i] = gen_read_in_code(
          read_in, racc_stmts[i], num_stmts_per_racc[i], prog, copy_level, l);
    }

    for (i = 0; i < num_read_write_data[l]; ++i) {
      ref_count_initialize_stmts[l][i] = gen_data_tile_ref_count_code(
          acc_stmts[i], num_stmts_per_acc[i], prog, copy_level, l);
    }

    for (i = 0; i < num_write_data[l]; ++i) {
      ref_count_writeout_stmts[l][i] = gen_write_out_tiles_ref_count_code(
  ...
  for (i = 0; i < prog->narrays; ++i) {
    ...
    null_init_stmts[i] = gen_null_init_code(arr, prog, 0);
  }
  ...
  for (l = 0; l < nloops; ++l) {
    for (i = 0; i < num_read_write_data[l]; ++i) {
      for (k = 0; k < num_ref_count_init_stmts; ++k) {
        pluto_add_given_stmt(data_tile_prog,
                             ref_count_initialize_stmts[l][i][k]);
        num_inital_stmts++;
      }
    }
  ...
  gen_func_cloog_code(func_name, headerfp, 0, 0, num_data, accs, data_tile_prog,
                      prog, 0);
...
int pluto_sharedmem_data_dist_codegen(PlutoProg *prog, Ploop **loops,
  ...
  for (l = 0; l < nloops; l++) {
    ...
    Stmt *alloc_data_tile =
        create_helper_stmt(anchor_stmt, copy_level[l], stmt_text, ORIG, l);
    ...
    Stmt *update_ref_count =
        create_helper_stmt(anchor_stmt, copy_level[l], stmt_text, ORIG, l);

    pluto_add_given_stmt(prog, update_ref_count);
    sep_stmts[l][count++] = update_ref_count;

    pluto_separate_stmts(prog, sep_stmts[l], sep_stmts_size[l], copy_level[l],
                         l);
  }
  ...
  fprintf(outfp, "\n\tIF_TIME(fprintf(stdout, \"Computation time: %%0.6lf "
                 "s\\n\", t_comp));\n\n");
...
int pluto_distmem_parallelize(PlutoProg *prog, FILE *sigmafp, FILE *headerfp,
  ...
  Stmt ***sep_stmts;
  int *pi_mappings, *num_write_data, *num_read_data, *num_read_write_data,
      *copy_level;
...
#define min(x,y)    ((x) < (y)? (x) : (y))\n\n");
  ...
  pluto_dist_gen_compute_pi(nloops, loops, copy_level, prog, pi_mappings, pifp,
                            headerfp);
  ...
  for (l = 0; l < nloops; l++) {
    ...
    if (options->data_dist) {
      for (i = 0; i < num_read_write_data[l]; i++) {
        ...
        generate_sigma(acc_stmts[i], num_stmts_per_acc[i], copy_level, prog, l,
                       pi_mappings, sigmafp, headerfp);

        if (options->commopt_fop) {
          copy_comm_stmts[l][i] = gen_comm_code_opt_fop(
              i, acc_stmts[i], num_stmts_per_acc[i], nloops,
              num_read_write_data[l], prog, anchor_stmt, copy_level,
              outer_dist_loop_level[l], l, pi_mappings, &num_comm_stmts,
              sigmafp, headerfp);

        } else if (options->commopt_foifi) {
          copy_comm_stmts[l][i] = gen_comm_code_opt_foifi(
              i, acc_stmts[i], num_stmts_per_acc[i], nloops,
              num_read_write_data[l], prog, anchor_stmt, copy_level,
              outer_dist_loop_level[l], l, pi_mappings, &num_comm_stmts,
              headerfp);
          ...
          copy_comm_stmts[l][i] =
              gen_comm_code_opt(i, acc_stmts[i], num_stmts_per_acc[i], nloops,
      ...
      if (options->verify_output) {
        for (i = 0; i < num_write_data[l]; i++) {
          ...
          gen_data_tile_copy_back_cloog_code(prog, l, wacc_stmts[i],
                                             num_stmts_per_wacc[i],
                                             copy_level[l], headerfp);
        }
        for (i = 0; i < num_write_data[l]; i++) {
          write_out_stmts[l][i] = gen_write_out_code(
              wacc_stmts[i], num_stmts_per_wacc[i], prog, anchor_stmt,
              copy_level, outer_dist_loop_level[l], l, headerfp);
        }
      }
    } else {
      for (i = 0; i < num_write_data[l]; i++) {
        generate_sigma(wacc_stmts[i], num_stmts_per_wacc[i], copy_level, prog,
                       l, pi_mappings, sigmafp, headerfp);
        if (options->commopt_fop) {
          copy_comm_stmts[l][i] = gen_comm_code_opt_fop(
              i, wacc_stmts[i], num_stmts_per_wacc[i], nloops, num_data[l],
              prog, anchor_stmt, copy_level, outer_dist_loop_level[l], l,
          ...
          copy_comm_stmts[l][i] = gen_comm_code_opt(
              i, wacc_stmts[i], num_stmts_per_wacc[i], nloops, num_data[l],
              prog, anchor_stmt, copy_level, outer_dist_loop_level[l], l,
              pi_mappings, &num_comm_stmts, headerfp);
        }
        write_out_stmts[l][i] = gen_write_out_code(
            wacc_stmts[i], num_stmts_per_wacc[i], prog, anchor_stmt, copy_level,
            outer_dist_loop_level[l], l, headerfp);
      }
    }
    ...
    if (options->dynschedule) {
      ...
      if (options->data_dist) {
        gen_pack_send_text_code(prog, copy_comm_stmts[l], acc_stmts, l,
                                num_read_write_data[l], num_comm_stmts,
                                copy_level[l], headerfp, loop);
        gen_unpack_text_code(prog, copy_comm_stmts[l], acc_stmts, l,
                             num_read_write_data[l], num_comm_stmts,
                             copy_level[l], headerfp);
      } else {
        gen_pack_send_text_code(prog, copy_comm_stmts[l], wacc_stmts, l,
                                num_data[l], num_comm_stmts, copy_level[l],
                                headerfp, loop);
        gen_unpack_text_code(prog, copy_comm_stmts[l], wacc_stmts, l,
                             num_data[l], num_comm_stmts, copy_level[l],
                             headerfp);
      }
    }
  ...
  for (l = 0; l < nloops; l++) {
    ...
    for (i = 0; i < num_scalar_dimensions_added; i++) {
      ...
      if (orig_dimension >= outer_dist_loop_level[l]) {
        // update the depth of the loops after scalar dimensions were added by
        // the previous loops
        // i represents the number of dimensions added before original depth of
        // the loop
    ...
    if (i == num_scalar_dimensions_added) {
      // update the depth of the loops after scalar dimensions were added by the
      // previous loops
      // i represents the number of dimensions added before original depth of
      // the loop
      outer_dist_loop_level[l] += i;
    ...
    if (options->verify_output)
      pluto_separate_stmts(write_out_prog, sep_writeout_stmts[l],
                           NUM_WRITE_OUT_STMTS * num_data[l],
  ...
  for (l = 0; l < nloops; l++) {
    ...
    /* Generate pi */
    generate_pi(pifp, headerfp, outer_dist_loop_level[l], inner_dist_loop_level,
                prog, loops[l], l, scalar_dimensions_added,
                num_scalar_dimensions_added_before);
  }
