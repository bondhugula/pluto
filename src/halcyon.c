 ...
 */
void generate_pi(FILE *outfp, int copy_level, const PlutoProg *prog,
...
#define min(x,y)    ((x) < (y)? (x) : (y))\n\n");
  ...
  int index_to_skip = 0;
  int index_offset = 1;
  index_offset++; // due to the scalar dimension that will be added to separate
                  // write-out statements from the rest
  ...
  copy_level++; // due to the scalar dimension that will be added to separate
                // write-out statements from the rest
  fprintf(outfp, "\
            int __p, __lb, __ub;\n\
          ...
          "_LB_REPLACE_ME_DISTLOOG", loop_num, "_UB_REPLACE_ME_DISTLOOG",
          loop_num, copy_level, copy_level);
}

void pluto_gen_sigma_cloog_code(const PlutoProg *sigma, FILE *cloogfp,
                                FILE *outfp, int partition_id) {
  int i;
  ...
  while (curr != NULL) {
    ...
    if (!broadcast_of_partition[partition_id]) {
      for (i = 0; i < num_dest_loops_of_partition[partition_id]; i++) {
        ...
        char *sigma_text;
        sigma_text =
            malloc(strlen("add_proc_to_receiver_list(pi_(, nprocs),my_rank)") +
                   5 + strlen(indices) + 1);
        sprintf(sigma_text,
                "add_proc_to_receiver_list(pi_%d(%s, nprocs),my_rank)",
    ...
    if (broadcast_of_partition[partition_id]) {
      fprintf(outfp, "int __p;\nfor (__p=0; __p<nprocs; __p++) "
                     "add_proc_to_receiver_list(__p, my_rank);\n");
    } else {
...
void generate_sigma(struct stmt_access_pair **wacc_stmts, int naccs,
  ...
  int dep_loop_num_list[prog->ndeps];
  int num_dest_loops = 0;
  int broadcast = !options->commopt; // nocommopt => no precise determination of
                                     // receivers => broadcast
  ...
  if (!broadcast) {
    for (i = 0; i < prog->ndeps; i++) {
      ...
      for (j = 0; j < naccs; j++) {
        if (src->writes[0] == wacc_stmts[j]->acc)
          break;
      }
      if (j == naccs)
        continue;
 ...
 * parameters along with global parameters
 */
void generate_tau(struct stmt_access_pair *wacc_stmt, int copy_level,
                  PlutoProg *prog) {
  int i, j;
  ...
  for (i = 0; i < prog->ndeps; i++) {
    ...
    pluto_constraints_project_out(tdpoly, copy_level,
                                  src->trans->nrows - copy_level);
    pluto_constraints_project_out(tdpoly, 2 * copy_level,
    ...
    pluto_matrix_add_col(trans, trans->ncols);
    char *tau_text =
        malloc(strlen("add_proc_to_sender_list(pi_table,my_rank)") +
               strlen(indices) + 1);
...
#define min(x,y)    ((x) < (y)? (x) : (y))\n\n");
  ...
  for (i = 0; i < copy_level + prog->npar; i++) {
    ...
    if (i <= copy_level - 1)
      ...
      fprintf(outfp, "int %s", prog->params[i - copy_level]);
  }
  fprintf(outfp, ", int my_rank, int nprocs)\n{\n");
