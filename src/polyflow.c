 ...
 * parameters along with global parameters; \pi(those inner_dist_loop_level
 *rows,
 * global params, nprocs) is what is constructed
 *
 * loop_num: parallel loop number
 */
void generate_pi(FILE *outfp, FILE *headerfp, int outer_dist_loop_level,
                 int inner_dist_loop_level, const PlutoProg *prog, Ploop *loop,
                 int loop_num, int *dimensions_to_skip,
                 int num_dimensions_to_skip) {
  int i;
  ...
  for (i = 0; i <= inner_dist_loop_level; i++) {
    if ((index_to_skip >= num_dimensions_to_skip) ||
        (i < dimensions_to_skip[index_to_skip])) {
      dims[num_dims++] = i;
      if (i >= outer_dist_loop_level) {
        if (options->dynschedule) {
          // since the dimensions-to-skip have not been added to the anchor
          // statement,
          ...
          if (!is_higher_loop_dim_seq) {
            if (pluto_is_hyperplane_loop(anchor_stmt, i)) {
              ...
              if (j == num_inner_loops) {
                ...
                is_higher_loop_dim_seq =
                    1; // should not distribute this loop, and loops inner to it
              }
            }
          }
        }
  ...
  for (i = 0; i < num_dist_dims; i++) {
    ...
    fprintf(outfp, "\
                ...
                __lb%d = %s%dt%d;\n\
                __ub%d = %s%dt%d;\n",
            i, i, i, i, "_LB_REPLACE_ME_DISTLOOG", loop_num, dim, i,
  ...
  case 1:
    ...
    fprintf(outfp, "\
            ...
            loop_num, dist_dims[0] + index_offset, dist_dims[0] + index_offset);
    break;
  case 2:
    fprintf(outfp, "\
#ifdef __AUTO_COMPUTE_PI\n\
  ...
  case 3:
    fprintf(outfp, "\
            ...
            dist_dims[0] + index_offset, dist_dims[1] + index_offset,
            dist_dims[2] + index_offset);
    break;
  default:
  ...
  if (options->distmem && options->dynschedule) {
    ...
    for (i = 0; i < num_dist_dims; i++) {
      ...
      fprintf(outfp, "\
              ...
              "_UB_REPLACE_ME_DISTLOOG", loop_num, dim, num_dist_dims, i);
      fprintf(outfp, "\
                    int __lb%d, __ub%d;\n\
                    __lb%d = lbp;\n\
                    __ub%d = ubp;\n",
  ...
  for (i = 0; i < num_dist_dims; i++) {
    fprintf(pidefsfp, "#define __DIM%dt%d %d\n", loop_num,
            dist_dims[i] + index_offset, i);
  }
  fprintf(pidefsfp, "#define __NUM_DIST_DIMS%d %d\n", loop_num, num_dist_dims);
  fclose(pidefsfp);
}

void pluto_gen_sigma_cloog_code(PlutoProg *sigma, FILE *cloogfp, FILE *outfp,
                                int partition_id) {
  int i;
...
/*
 * generate_sigma_dep_split: Generates receivers for a given tile for a
 * particular
 * stmt_access_pair
...
void generate_sigma_dep_split(struct stmt_access_pair **wacc_stmts, int naccs,
                              ...
                              PlutoConstraintsList *partitions, int loop_num,
  ...
  while (curr != NULL) {
    ...
    while (curr_dep != NULL) {
      ...
      PlutoConstraints *tdpoly =
          pluto_constraints_dup(dep->src_unique_dpolytope);
      ...
      pluto_constraints_project_out(tdpoly, src_copy_level,
                                    src->trans->nrows - src_copy_level);
      pluto_constraints_project_out(tdpoly, total_copy_level,
                                    dest->trans->nrows - dest_copy_level);
    ...
    for (j = 0; j < num_partitions; j++) {
      if (num_dest_loops_of_partition[j] == num_dest_loops) {
        for (i = 0; i < num_dest_loops; i++) {
          ...
          int constraints_are_equal =
              pluto_constraints_are_equal(tdpoly1, tdpoly2);
          pluto_constraints_free(tdpoly1);
          pluto_constraints_free(tdpoly2);
          if (!constraints_are_equal)
            break;
        }
        if (i == num_dest_loops) { // constraints of receiving tiles are equal
                                   // for all dest loops
          if (broadcast)
            broadcast_of_partition[j] = 1;
          for (i = 0; i < num_dest_loops;
               i++) { // because parameters are not considered for equality
            pluto_constraints_unionize(tdpoly_list_of_partition[j][i],
                                       tdpoly_list[i]);
          }
          pluto_constraints_unionize(partitionj->constraints,
                                     curr->constraints);
          PlutoDepList *dep_list =
              curr->deps; // !!!roshan may not need to copy/fuse deps
          while (dep_list != NULL) {
          ...
          for (i = 0; i < num_dest_loops; i++)
            pluto_constraints_free(tdpoly_list[i]);
    ...
    if (j == num_partitions) {
      ...
      if (broadcast)
        broadcast_of_partition[num_partitions] = 1;
      num_dest_loops_of_partition[num_partitions] = num_dest_loops;
      for (i = 0; i < num_dest_loops; i++) {
        dep_loop_num_list_of_partition[num_partitions][i] =
            dep_loop_num_list[i];
  ...
  while (curr != NULL) {
    ...
    if (!broadcast_of_partition[partition_id]) {
      for (i = 0; i < num_dest_loops_of_partition[partition_id]; i++) {
        ...
        for (j = 0; j < dest_copy_level + prog->npar; j++) {
          ...
          if (j <= dest_copy_level - 1) {
            ...
            sprintf(indices + strlen(indices), "%s",
                    prog->params[j - dest_copy_level]);
        }
        ...
        char *sigma_text;
        sigma_text = malloc(strlen("__p = pi_(,nprocs); if (__p != my_rank) { "
                                   "receiver_list[__p] = 1; }") +
                            5 + strlen(indices) + 1);
        sprintf(sigma_text, "__p = pi_%d(%s,nprocs); if (__p != my_rank) { "
                            "receiver_list[__p] = 1; }",
                dep_loop_num, indices);
        char *is_receiver_text;
        is_receiver_text =
            malloc(strlen("if (pi_(, nprocs) == my_rank) return 1;") + 5 +
                   strlen(indices) + 1);
        sprintf(is_receiver_text, "if (pi_%d(%s, nprocs) == my_rank) return 1;",
                dep_loop_num, indices);
        char *sigma_check_text = NULL;
        if (options->fop_unicast_runtime &&
            !broadcast_of_partition[partition_id]) {
          sigma_check_text = malloc(
              strlen("recv_proc = pi_(, nprocs); if (recv_proc != my_rank) { "
                     "if (receiver_list[recv_proc] > __FOP_UNICAST_RECV_LIMIT) "
                     "return 0; else receiver_list[recv_proc]++; }") +
              5 + strlen(indices) + 1);
          sprintf(sigma_check_text,
                  "recv_proc = pi_%d(%s, nprocs); if (recv_proc != my_rank) { "
                  "if (receiver_list[recv_proc] > __FOP_UNICAST_RECV_LIMIT) "
                  "return 0; else receiver_list[recv_proc]++; }",
                  dep_loop_num, indices);
        }
        ...
        pluto_add_stmt(is_receiver, tdpoly, trans, iters, is_receiver_text,
                       IN_FUNCTION);
        if (options->fop_unicast_runtime &&
            !broadcast_of_partition[partition_id])
          pluto_add_stmt(sigma_check, tdpoly, trans, iters, sigma_check_text,
                         IN_FUNCTION);
      ...
      if (options->fop_unicast_runtime && !broadcast_of_partition[partition_id])
        pluto_pad_stmt_transformations(sigma_check);
      ...
      pluto_separate_stmts(is_receiver, is_receiver->stmts, is_receiver->nstmts,
                           0, 0);
      if (options->fop_unicast_runtime && !broadcast_of_partition[partition_id])
        pluto_separate_stmts(sigma_check, sigma_check->stmts,
                             sigma_check->nstmts, 0, 0);
    }

    FILE *cloogfp = NULL;

    fprintf(outfp, "void sigma_%s_%d_%d(", acc_name, partition_id + 1,
            loop_num);
    fprintf(headerfp, "void sigma_%s_%d_%d(", acc_name, partition_id + 1,
            loop_num);
    ...
    if (broadcast_of_partition[partition_id]) {
      fprintf(outfp, "\nfor (__p=0; __p<nprocs; __p++) if (__p != my_rank) "
                     "receiver_list[__p] = 1;\n");
    } else {
    ...
    fprintf(outfp, "int is_receiver_%s_%d_%d(", acc_name, partition_id + 1,
            loop_num);
    fprintf(headerfp, "int is_receiver_%s_%d_%d(", acc_name, partition_id + 1,
            loop_num);
    for (i = 0; i < src_copy_level + prog->npar; i++) {
      if (i != 0) {
        fprintf(outfp, ", ");
    ...
    if (options->fop_unicast_runtime && !broadcast_of_partition[partition_id]) {
      fprintf(outfp, "int sigma_check_%s_%d_%d(", acc_name, partition_id + 1,
              loop_num);
      fprintf(headerfp, "int sigma_check_%s_%d_%d(", acc_name, partition_id + 1,
              loop_num);
      for (i = 0; i < src_copy_level + prog->npar; i++) {
        if (i != 0) {
          fprintf(outfp, ", ");
...
void generate_sigma_common(struct stmt_access_pair **wacc_stmts, int naccs,
  ...
  *broadcast =
      !(options->commopt_fop || options->commopt_foifi || options->commopt);
  ...
  if (!*broadcast) {
    for (i = 0; i < prog->ndeps; i++) {
      ...
      for (j = 0; j < naccs; j++) {
        if (dep->src_acc == wacc_stmts[j]->acc)
          break;
      }
      if (j == naccs)
        continue;
      ...
      if (src_copy_level == dest_copy_level) {
        PlutoConstraints *param_eq =
            get_context_equality(src_copy_level, src_copy_level, tdpoly->ncols);
    ...
    for (i = 0; i < *num_dest_loops; i++) {
      ...
      for (j = 0; j < dest_copy_level + prog->npar; j++) {
        ...
        if (j <= dest_copy_level - 1) {
          ...
          sprintf(indices[i] + strlen(indices[i]), "%s",
                  prog->params[j - dest_copy_level]);
      }
...
void generate_tau_common(struct stmt_access_pair **racc_stmts, int naccs,
                         int *copy_level, PlutoProg *prog, int loop_num,
                         int *pi_mappings, int *num_src_loops,
                         PlutoConstraints *tdpoly_list[prog->ndeps],
  ...
  *num_src_loops = 0;
  ...
  for (i = 0; i < prog->ndeps; i++) {
    ...
    for (j = 0; j < naccs; j++) {
      if (dep->dest_acc == racc_stmts[j]->acc)
        break;
    }
    if (j == naccs)
      continue;
...
void generate_outgoing_common(Stmt **loop_stmts, int nstmts, int *copy_level,
                              PlutoProg *prog, int loop_num, int *pi_mappings,
                              int isRemoteOnly, int *num_dest_loops,
                              PlutoConstraints *tdpoly_list[prog->ndeps],
                              int dep_loop_num_list[prog->ndeps],
                              PlutoMatrix *trans[prog->ndeps],
                              char **iters[prog->ndeps],
  ...
  *num_dest_loops = 0;
  ...
  int ndeps;
  Dep **deps;
  if (options->dyn_trans_deps_tasks && !isRemoteOnly && options->distmem &&
      options->lastwriter) {
  ...
  for (i = 0; i < *num_dest_loops; i++) {
    ...
    for (j = 0; j < num_dims; j++) {
      sprintf(task_indices[i] + strlen(task_indices[i]),
              "(%s-lb_tasks_loop%d_dim%d)", iters[i][dims[j]], dep_loop_num, j);
      for (k = j + 1; k < num_dims; k++) {
        sprintf(task_indices[i] + strlen(task_indices[i]),
                "*max_num_tasks_loop%d_dim%d", dep_loop_num, k);
      }
...
void generate_incoming_common(Stmt **loop_stmts, int nstmts, int *copy_level,
                              PlutoProg *prog, int loop_num, int *pi_mappings,
                              int isRemoteOnly, int *num_src_loops,
                              PlutoConstraints *tdpoly_list[prog->ndeps],
                              int src_loop_num_list[prog->ndeps],
                              PlutoMatrix *trans[prog->ndeps],
                              char **iters[prog->ndeps],
...
void generate_outgoing(Stmt **loop_stmts, int nstmts, int *copy_level,
  ...
  if (options->distmem) {
    ...
    for (i = 0; i < prog->npar; i++) {
      pluto_prog_add_param(count_remote_dep_tasks, prog->params[i],
                           count_remote_dep_tasks->npar);
    }
    ...
    for (i = 0; i < prog->npar; i++) {
      pluto_prog_add_param(remote_update_dep_tasks, prog->params[i],
                           remote_update_dep_tasks->npar);
    }
  }
  ...
  if (options->dynschedule) {
    ...
    for (i = 0; i < prog->npar; i++) {
      pluto_prog_add_param(count_local_dep_tasks, prog->params[i],
                           count_local_dep_tasks->npar);
    }
    ...
    for (i = 0; i < prog->npar; i++) {
      pluto_prog_add_param(local_update_dep_tasks, prog->params[i],
                           local_update_dep_tasks->npar);
    }
  }
  ...
  if (options->dynschedule_graph) {
    ...
    for (i = 0; i < prog->npar; i++) {
      pluto_prog_add_param(add_outgoing_edges, prog->params[i],
                           add_outgoing_edges->npar);
    }
  }
  ...
  if (options->distmem) {
    ...
    generate_outgoing_common(loop_stmts, nstmts, copy_level, prog, loop_num,
                             pi_mappings, 1, &num_dest_loops, tdpoly_list,
                             dep_loop_num_list, trans, iters, indices,
                             task_indices);
    ...
    for (i = 0; i < num_dest_loops; i++) {
      ...
      is_receiver_text =
          malloc(strlen("if (pi_(,, nprocs) == my_rank) return 1;") + 5 +
                 strlen(indices[i]) + strlen(params) + 1);
      sprintf(is_receiver_text,
              "if (pi_%d(%s,%s, nprocs) == my_rank) return 1;", dep_loop_num,
              indices[i], params);
      pluto_add_stmt(is_receiver, tdpoly, trans[i], iters[i], is_receiver_text,
                     IN_FUNCTION);

      char *count_remote_dep_tasks_text = NULL;
      count_remote_dep_tasks_text =
          malloc(strlen("if (pi_(,, nprocs) != my_rank) count++;") + 5 +
                 strlen(indices[i]) + strlen(params) + 1);
      sprintf(count_remote_dep_tasks_text,
              "if (pi_%d(%s,%s, nprocs) != my_rank) count++;", dep_loop_num,
              indices[i], params);
      pluto_add_stmt(count_remote_dep_tasks, tdpoly, trans[i], iters[i],
                     count_remote_dep_tasks_text, IN_FUNCTION);

      char *remote_update_dep_tasks_text = malloc(2048);
      sprintf(remote_update_dep_tasks_text,
              ...
              params, dep_loop_num, dep_loop_num, dep_loop_num, indices[i],
              params, dep_loop_num, indices[i]);
      sprintf(remote_update_dep_tasks_text +
                  strlen(remote_update_dep_tasks_text),
      ...
      sprintf(remote_update_dep_tasks_text +
              ...
              indices[i]);
      pluto_add_stmt(remote_update_dep_tasks, tdpoly, trans[i], iters[i],
                     remote_update_dep_tasks_text, IN_FUNCTION);
    ...
    if (is_receiver->nstmts >= 1) {
      assert(is_receiver->stmts[0]->trans->nrows ==
             is_receiver->num_hyperplanes);
    }
    pluto_separate_stmts(is_receiver, is_receiver->stmts, is_receiver->nstmts,
                         0, 0);
    ...
    if (count_remote_dep_tasks->nstmts >= 1) {
      assert(count_remote_dep_tasks->stmts[0]->trans->nrows ==
             count_remote_dep_tasks->num_hyperplanes);
    }
    pluto_separate_stmts(count_remote_dep_tasks, count_remote_dep_tasks->stmts,
                         count_remote_dep_tasks->nstmts, 0, 0);

    pluto_pad_stmt_transformations(remote_update_dep_tasks);
    if (remote_update_dep_tasks->nstmts >= 1) {
      assert(remote_update_dep_tasks->stmts[0]->trans->nrows ==
             remote_update_dep_tasks->num_hyperplanes);
    }
    pluto_separate_stmts(remote_update_dep_tasks,
                         remote_update_dep_tasks->stmts,
                         remote_update_dep_tasks->nstmts, 0, 0);
  }

  {
    ...
    generate_outgoing_common(
        loop_stmts, nstmts, copy_level, prog, loop_num, pi_mappings, 0,
        &local_num_dest_loops, local_tdpoly_list, local_dep_loop_num_list,
        local_trans, local_iters, local_indices, local_task_indices);
    ...
    for (i = 0; i < local_num_dest_loops; i++) {
      ...
      if (options->dynschedule) {
        if (options->distmem) {
          count_local_dep_tasks_text =
              malloc(strlen("if (pi_(,, nprocs) == my_rank) count++;") + 5 +
                     strlen(local_indices[i]) + strlen(params) + 1);
          sprintf(count_local_dep_tasks_text,
                  "if (pi_%d(%s,%s, nprocs) == my_rank) count++;", dep_loop_num,
                  local_indices[i], params);
        } else {
          count_local_dep_tasks_text = malloc(16);
          strcpy(count_local_dep_tasks_text, "count++;");
        }
        pluto_add_stmt(count_local_dep_tasks, tdpoly, local_trans[i],
                       local_iters[i], count_local_dep_tasks_text, IN_FUNCTION);
      ...
      if (options->dynschedule) {
        ...
        if (options->distmem) {
          sprintf(
              ...
              "__omp_atomic_capture captured_firing_count = --tasks_loop%d%s; \
                            ...
                            __Task task(%d,%s,remote_dep_tasks,local_dep_tasks,affinity); \
                            pqueue.push(task); ",
              dep_loop_num, local_task_indices[i], dep_loop_num,
              local_indices[i], params, dep_loop_num, local_indices[i], params,
              dep_loop_num, local_indices[i], params, dep_loop_num,
              dep_loop_num, dep_loop_num, local_indices[i], params,
              dep_loop_num, local_indices[i]);
          sprintf(local_update_dep_tasks_text +
                      strlen(local_update_dep_tasks_text),
                  "IF_DYNSCHEDULER_MORE_DEBUG_PRINT(\
                            fprintf(__debug_print_fp, \"added node %%d loop %d task ",
                  dep_loop_num);
          for (j = 0; j < dest_copy_level; j++) {
        ...
        pluto_add_stmt(local_update_dep_tasks, tdpoly, local_trans[i],
                       local_iters[i], local_update_dep_tasks_text,
                       IN_FUNCTION);
      ...
      if (options->dynschedule_graph) {
        ...
        pluto_add_stmt(add_outgoing_edges, tdpoly, local_trans[i],
                       local_iters[i], add_outgoing_edges_text, IN_FUNCTION);
    ...
    if (options->dynschedule) {
      ...
      if (count_local_dep_tasks->nstmts >= 1) {
        assert(count_local_dep_tasks->stmts[0]->trans->nrows ==
               count_local_dep_tasks->num_hyperplanes);
      }
      pluto_separate_stmts(count_local_dep_tasks, count_local_dep_tasks->stmts,
                           count_local_dep_tasks->nstmts, 0, 0);

      pluto_pad_stmt_transformations(local_update_dep_tasks);
      if (local_update_dep_tasks->nstmts >= 1) {
        assert(local_update_dep_tasks->stmts[0]->trans->nrows ==
               local_update_dep_tasks->num_hyperplanes);
      }
      pluto_separate_stmts(local_update_dep_tasks,
                           local_update_dep_tasks->stmts,
                           local_update_dep_tasks->nstmts, 0, 0);
    }

    if (options->dynschedule_graph) {
      pluto_pad_stmt_transformations(add_outgoing_edges);
      if (add_outgoing_edges->nstmts >= 1) {
        assert(add_outgoing_edges->stmts[0]->trans->nrows ==
               add_outgoing_edges->num_hyperplanes);
      }
      pluto_separate_stmts(add_outgoing_edges, add_outgoing_edges->stmts,
                           add_outgoing_edges->nstmts, 0, 0);
    }
  }
  free(params);
  ...
  if (options->distmem) {
    ...
    fprintf(headerfp, ", int my_rank, int nprocs, int num_threads%s);\n",
            tasks_loops_decl);
    fprintf(outfp, "int remote_dep_tasks, local_dep_tasks, affinity, "
                   "captured_firing_count;\n");
  ...
  if (options->dynschedule_graph) {
    ...
    fprintf(outfp, "%s, tbb::flow::continue_node<tbb::flow::continue_msg> "
                   "*current_task)\n{\n",
            tasks_loops_decl);
    fprintf(headerfp, "%s, tbb::flow::continue_node<tbb::flow::continue_msg> "
                      "*current_task);\n",
            tasks_loops_decl);
    fprintf(outfp, "\nint count = 0;\n");
...
void generate_incoming(Stmt **loop_stmts, int nstmts, int *copy_level,
  ...
  if (options->distmem) {
    ...
    for (i = 0; i < prog->npar; i++) {
      pluto_prog_add_param(count_remote_src_tasks, prog->params[i],
                           count_remote_src_tasks->npar);
    }
  }
  ...
  for (i = 0; i < prog->npar; i++) {
    pluto_prog_add_param(count_local_src_tasks, prog->params[i],
                         count_local_src_tasks->npar);
  }
  ...
  if (options->distmem) {
    ...
    // for remote dependences only i.e., RAW dependences only
    generate_incoming_common(loop_stmts, nstmts, copy_level, prog, loop_num,
                             pi_mappings, 1, &num_src_loops, tdpoly_list,
                             src_loop_num_list, trans, iters, indices);
    ...
    for (i = 0; i < num_src_loops; i++) {
      ...
      count_remote_src_tasks_text =
          malloc(strlen("if (pi_(, nprocs) != my_rank) count;") + 5 +
                 strlen(indices[i]) + 1);
      sprintf(count_remote_src_tasks_text,
              "if (pi_%d(%s, nprocs) != my_rank) count++;", src_loop_num,
              indices[i]);
      pluto_add_stmt(count_remote_src_tasks, tdpoly, trans[i], iters[i],
                     count_remote_src_tasks_text, IN_FUNCTION);
    ...
    if (count_remote_src_tasks->nstmts >= 1) {
      assert(count_remote_src_tasks->stmts[0]->trans->nrows ==
             count_remote_src_tasks->num_hyperplanes);
    }
    pluto_separate_stmts(count_remote_src_tasks, count_remote_src_tasks->stmts,
                         count_remote_src_tasks->nstmts, 0, 0);
  }

  {
    ...
    // for all memory dependences only i.e., RAW, WAW, WAR dependences
    generate_incoming_common(loop_stmts, nstmts, copy_level, prog, loop_num,
                             pi_mappings, 0, &local_num_src_loops,
                             local_tdpoly_list, local_src_loop_num_list,
                             local_trans, local_iters, local_indices);
    ...
    for (i = 0; i < local_num_src_loops; i++) {
      ...
      if (options->distmem) {
        count_local_src_tasks_text =
            malloc(strlen("if (pi_(, nprocs) == my_rank) count;") + 5 +
                   strlen(local_indices[i]) + 1);
        sprintf(count_local_src_tasks_text,
                "if (pi_%d(%s, nprocs) == my_rank) count++;", src_loop_num,
                local_indices[i]);
      } else {
      ...
      pluto_add_stmt(count_local_src_tasks, tdpoly, local_trans[i],
                     local_iters[i], count_local_src_tasks_text, IN_FUNCTION);
    ...
    if (count_local_src_tasks->nstmts >= 1) {
      assert(count_local_src_tasks->stmts[0]->trans->nrows ==
             count_local_src_tasks->num_hyperplanes);
    }
    pluto_separate_stmts(count_local_src_tasks, count_local_src_tasks->stmts,
                         count_local_src_tasks->nstmts, 0, 0);
  }
...
void generate_sigma(struct stmt_access_pair **wacc_stmts, int naccs,
  ...
  int broadcast;
  {
    ...
    generate_sigma_common(wacc_stmts, naccs, copy_level, prog, loop_num,
                          ...
                          tdpoly_list, dep_loop_num_list, trans, iters,
                          indices);
    ...
    if (!broadcast) {
      for (i = 0; i < num_dest_loops; i++) {
        ...
        if (is_sigma_required) {
          ...
          pluto_add_stmt(sigma, tdpoly, trans[i], iters[i], sigma_text,
                         IN_FUNCTION);
        }
        ...
        if (is_receiver_required) {
          is_receiver_text =
              malloc(strlen("if (pi_(, nprocs) == my_rank) return 1;") + 5 +
                     strlen(indices[i]) + 1);
          sprintf(is_receiver_text,
                  "if (pi_%d(%s, nprocs) == my_rank) return 1;", dep_loop_num,
                  indices[i]);
          pluto_add_stmt(is_receiver, tdpoly, trans[i], iters[i],
                         is_receiver_text, IN_FUNCTION);
        }
      ...
      if (is_receiver_required) {
        ...
        pluto_separate_stmts(is_receiver, is_receiver->stmts,
                             is_receiver->nstmts, 0, 0);
      }
    }
  }
...
PlutoConstraints *get_receiver_tiles_of_dep(Dep *dep, int src_copy_level,
  ...
  PlutoConstraints *tdpoly;
  if (!use_src_unique_dpolytope)
    tdpoly = pluto_get_transformed_dpoly(dep, src, dest);
  else
    tdpoly = pluto_constraints_dup(dep->src_unique_dpolytope);
  assert(tdpoly->ncols ==
         src->trans->nrows + dest->trans->nrows + prog->npar + 1);
...
/*
 * get_receiver_tiles: Returns receiver tiles for the given stmt_access_pairs
 * and
...
PlutoConstraints *get_receiver_tiles(struct stmt_access_pair **wacc_stmts,
  ...
  PlutoConstraints *receiver_tiles =
      pluto_constraints_empty(total_copy_level + prog->npar + 1);
  for (i = 0; i < prog->ndeps; i++) {
    Dep *dep = prog->deps[i];
    /* Only RAW deps matter */
    if (dep->type != OSL_DEPENDENCE_RAW)
    ...
    if (pi_mappings[dest->id] != dep_loop_num)
      continue;
    ...
    PlutoConstraints *tdpoly = get_receiver_tiles_of_dep(
        dep, src_copy_level, dest_copy_level, prog, 0);
...
void generate_tau(struct stmt_access_pair **racc_stmts, int naccs,
  ...
  generate_tau_common(racc_stmts, naccs, copy_level, prog, loop_num,
                      pi_mappings, &num_src_loops, tdpoly_list,
                      src_loop_num_list, trans, iters, indices);
  ...
  for (i = 0; i < num_src_loops; i++) {
    ...
    count_sending_tasks_text =
        malloc(strlen("if (pi_(,nprocs) != my_rank) count++;") + 5 +
               strlen(indices[i]) + 1);
    sprintf(count_sending_tasks_text,
            "if (pi_%d(%s,nprocs) != my_rank) count++;", src_loop_num,
            indices[i]);
    pluto_add_stmt(count_sending_tasks, tdpoly, trans[i], iters[i],
                   count_sending_tasks_text, IN_FUNCTION);
  ...
  if (count_sending_tasks->nstmts >= 1) {
    assert(count_sending_tasks->stmts[0]->trans->nrows ==
           count_sending_tasks->num_hyperplanes);
  }

  pluto_separate_stmts(count_sending_tasks, count_sending_tasks->stmts,
                       count_sending_tasks->nstmts, 0, 0);
...
void pluto_constraints_list_dep_add(PlutoConstraintsList *list, Dep *dep) {
  ...
  PlutoConstraintsList *new =
      pluto_constraints_list_alloc(pluto_constraints_dup(dep->dpolytope));
  new->next = list->next;
  list->next = new;
...
*/
void pluto_constraints_list_add_flowout(PlutoConstraintsList *list,
...
Dep *pluto_dependence_dup(Dep *dep, PlutoConstraints *tdpoly,
...
void compute_common_data_iterations(PlutoConstraints *td1,
                                    PlutoConstraints *td2, Dep *d1, Dep *d2,
                                    PlutoProg *prog,
                                    PlutoConstraints **intersect1,
                                    PlutoConstraints **intersect2) {
  ...
  if (0) {
    ...
    *intersect1 = pluto_constraints_intersection(td1, td2);
    ...
    // try finding the dependence between source iterators with their respective
    // access functions
    PlutoConstraints *tdpoly =
        pluto_find_dependence(td1, td2, d1, d2, prog, NULL);
...
// Recursively do the dependence intersection and difference on all the elements
// of list with dep_to_split and put the
// intersected dependence in new list
void intersect_deps_in_list(PlutoDepList *list, PlutoDepList *intersect_list,
                            Dep *dep_to_split, PlutoProg *prog) {
  ...
  compute_common_data_iterations(td1, td2, dep_to_split, curr_dep, prog,
...
int check_for_disjoint_sets(PlutoConstraintsList *list) {
  ...
  while (outer != NULL) {
    inner = outer->next;
    while (inner != NULL) {
      if (!pluto_constraints_is_empty(pluto_constraints_intersection(
               outer->constraints, inner->constraints)))
        return 0;
...
int check_for_valid_data(PlutoDepListList *list, int copy_level,
                         PlutoProg *prog) {
  ...
  while (curr != NULL && curr->dep_list != NULL) {
    ...
    data =
        pluto_compute_region_data(src, tdpoly, dep->src_acc, copy_level, prog);
    ...
    while (d != NULL) {
      ...
      datac = pluto_compute_region_data(src, tdpoly, dep->src_acc, copy_level,
                                        prog);

      IF_DEBUG(print_polylib_visual_sets("data2", datac));

      if (!pluto_constraints_is_empty(
               pluto_constraints_difference(datac, data))) {
        return 0;
      }
...
void split_dependences(PlutoDepListList *dep_list_list,
                       PlutoConstraints *curr_dep_poly, Dep *dep,
                       PlutoProg *prog) {
  ...
  */
  ...
  while (curr != NULL) {
    ...
    if (diff1_empty && diff2_empty) {
      ...
      // Add the new dep list for the new intersected deps which access same
      // data
  ...
  // If it is dep that has some source iterators that do not intersect with any
  // dep in list
  // add them to list
...
void split_flow_out_set(PlutoProg *prog, PlutoConstraintsList *atomic_flowouts,
  ...
  */
  ...
  while ((!curr_flowout_empty) && (curr != NULL)) {
    ...
    pluto_constraints_project_out(cst1, cst1->ncols - prog->npar - 1,
                                  prog->npar);
    cst2 = pluto_constraints_dup(curr_flowout);
    pluto_constraints_project_out(cst2, cst2->ncols - prog->npar - 1,
                                  prog->npar);
    ...
    intersect =
        pluto_constraints_intersection(cst_intersect, curr->constraints);
...
PlutoConstraints *compute_flow_in_of_dep(Dep *dep, int copy_level,
   ...
   */
  param_eq = get_context_equality(copy_level, src->trans->nrows, tdpoly->ncols);
  ...
  if (pluto_constraints_is_empty(tile_src_inside)) {
    ...
    // target and source statements could still have the same set of surrounding
    // loops
    // so incorrect to project out source copy_level loop iterators - retain
    // them
    ...
    for (j = 0; j < copy_level; j++) {
      pluto_constraints_interchange_cols(tile_src, j,
                                         j + copy_level + src->trans->nrows);
    }
    for (j = 0; j < copy_level; j++) {
    ...
    scst =
        pluto_compute_region_data(src, tile_src, write_acc, copy_level, prog);
  } else { // some source iterations within the source tile
    // target and source statements definitely have outer copy_level surrounding
    // loops common
    // so project out the source copy_level iterators too

    // project out source copy_level loop iterators and target loop iterators
    // inner to copy_level
    pluto_constraints_project_out(tile_src_inside, 0, copy_level);
    ...
    // project out source copy_level loop iterators and target loop iterators
    // inner to copy_level
    pluto_constraints_project_out(tile_src, 0, copy_level);
    pluto_constraints_project_out(tile_src, src->trans->nrows,
                                  dest->trans->nrows - copy_level);

    // difference should be taken after the two sets are parameterized on the
    // target copy_level iterators
    PlutoConstraints *tile_src_outside =
        pluto_constraints_difference(tile_src, tile_src_inside);
    ...
    for (j = 0; j < copy_level; j++) {
      pluto_constraints_interchange_cols(tile_src_outside, j,
                                         j + src->trans->nrows);
    }
    for (j = 0; j < copy_level; j++) {
...
PlutoConstraints *compute_flow_in(struct stmt_access_pair *racc_stmt,
  ...
  PlutoConstraints *uscst =
      pluto_constraints_empty(copy_level + racc->mat->nrows + prog->npar + 1);
...
PlutoConstraints *compute_flow_out_of_dep(Dep *dep, int src_copy_level,
   ...
   */
  param_eq =
      get_context_equality(src_copy_level, src->trans->nrows, tdpoly->ncols);
  ...
  if (pluto_constraints_is_empty(tile_dest_inside)) {
    ...
    // target and source statements could still have the same set of surrounding
    // loops
    // so incorrect to project out target src_copy_level loop iterators - retain
    // them
    ...
    if (split) {
      ...
      tile_dest_same_copy_level =
          pluto_constraints_intersection(tile_dest, copyleveleq);

      if (!pluto_constraints_is_empty(tile_dest_same_copy_level)) {
        /* dcst1 is parameterized by the source src_copy_level iterators */
        *dcst1 = pluto_compute_region_data(dest, tile_dest_same_copy_level,
                                           read_acc, src_copy_level, prog);
    ...
    // target and source statements definitely have outer src_copy_level
    // surrounding loops common
    // so the target src_copy_level iterators can be projected out
    // !!!roshan not sure if projecting out target src_copy_level iterators is
    // necessary
    // project out target src_copy_level loop iterators and source loop
    // iterators inner to src_copy_level
    pluto_constraints_project_out(tile_dest_inside, src_copy_level,
                                  src->trans->nrows);
    pluto_constraints_project_out(tile_dest, src_copy_level, src->trans->nrows);

    // difference should be taken after the two sets are parameterized on the
    // source src_copy_level iterators
    ...
    for (j = 0; j < src->trans->nrows; j++)
      pluto_constraints_add_dim(dep->src_unique_dpolytope, src_copy_level,
                                NULL);
    pluto_constraints_intersect(dep->src_unique_dpolytope, tdpoly);
  }

  /* dcst is parameterized by the source src_copy_level iterators */
  dcst = pluto_compute_region_data(dest, tile_dest, read_acc, src_copy_level,
                                   prog);
...
PlutoConstraints *compute_flow_out(struct stmt_access_pair *wacc_stmt,
  ...
  PlutoConstraints *udcst = pluto_constraints_empty(
      src_copy_level + wacc->mat->nrows + prog->npar + 1);
  ...
  for (i = 0; i < prog->ndeps; i++) {
    ...
    PlutoConstraints *dcst = compute_flow_out_of_dep(
...
void split_deps_acc_flowout(PlutoConstraintsList *atomic_flowouts,
  ...
  while (curr != NULL && curr->constraints != NULL) {
    ...
    // Create a identity access function for data with outer copy level
    // parameters
    ...
    while (dep_list != NULL) {
      ...
      Dep *dep = pluto_dependence_dup(dep_list->dep, dep_list->dep->dpolytope,
                                      dep_list->dep->src_unique_dpolytope);
      ...
      PlutoConstraints *src_iterators =
          pluto_constraints_dup(dep->src_unique_dpolytope);
      ...
      PlutoConstraints *tdpoly = pluto_find_dependence(src_iterators, data, dep,
                                                       dep, prog, data_access);

      if (tdpoly != NULL) {
        PlutoConstraints *parm_eq =
            get_context_equality(copy_level, src_ncols, tdpoly->ncols);
        ...
        /*
                    ...
                    //pluto_constraints_project_out(intersect,
           copy_level+access_nrows, src_ncols);
                    pluto_constraints_project_out(intersect, 0, src_ncols);

                    IF_DEBUG(print_polylib_visual_sets("int", intersect));

                    PlutoConstraints *diff =
           pluto_constraints_difference(intersect, data);
                    assert(pluto_constraints_is_empty(diff));

                    //IF_DEBUG(print_polylib_visual_sets("tdpoly", tdpoly));

                    //pluto_constraints_project_out(tdpoly, 0, copy_level +
           access_nrows);

        */
        pluto_constraints_project_out(tdpoly, src_ncols,
                                      copy_level + access_nrows);
      ...
      if ((tdpoly == NULL) ||
          pluto_constraints_is_empty(dep->src_unique_dpolytope)) {
...
void compute_flow_out_partitions(struct stmt_access_pair *wacc_stmt,
  ...
  for (i = 0; i < prog->ndeps; i++) {
    ...
    PlutoConstraints *dcst = compute_flow_out_of_dep(
        dep, src_copy_level, copy_level, prog, 1, &dcst1, pi_mappings);
...
PlutoConstraints *compute_read_in(struct stmt_access_pair *racc_stmt,
  ...
  PlutoConstraints *urcst =
      pluto_compute_region_data(rstmt, srcdomain, racc, copy_level, prog);
  ...
  for (i = 0; i < prog->ndeps; i++) {
    ...
    if (dep->type != OSL_DEPENDENCE_RAR)
      continue;
     ...
     */
    PlutoConstraints *rcst = pluto_compute_region_data(
        src, tile_dest_outside, racc_src, copy_level, prog);
