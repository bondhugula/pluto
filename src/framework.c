...
PlutoConstraints **get_array_ortho_constraints(Array *arr,
  ...
  int start_arr = (npar + 1) * (narrays + 1) +
                  (prog->nvar + npar + 1) * prog->nstmts +
  ...
  *orthonum = p;
  ...
  IF_DEBUG2(
      printf("Ortho constraints for %s; %d sets\n", arr->text, *orthonum));
...
PlutoConstraints *get_access_constraints(PlutoAccess *access, int mod_pos,
  ...
  int cst_ncols =
      (prog->npar + 1) + stmt_cst_width + arr_cst_width + num_farkas_mul + 1;
  ...
  for (i = 0; i < mat->ncols; i++) {
    ...
    // for program parameters and const we should set bounding func coeff u1,
    // u2, u3 ... & w
...
void pluto_constraints_selctive_copy(PlutoConstraints *from,
                                     PlutoConstraints *to, int ncols,
...
PlutoConstraints *pluto_dist_add_constraints_array_access(PlutoAccess *access,
  ...
  int cst_ncols = (prog->npar + 1) * (prog->narrays + 1) +
  ...
  start_stmt =
      (prog->npar + 1) * (prog->narrays + 1) + stmt->id * stmt_cst_width;

  // Search the find the array id
  for (k = 0; k < prog->narrays; k++) {
    if (!strcmp(prog->arrays[k]->text, access->name)) {
      start_arr = (prog->npar + 1) * (prog->narrays + 1) +
                  prog->nstmts * stmt_cst_width +
                  prog->arrays[k]->id * arr_cst_width;
  ...
  pluto_constraints_selctive_copy(cst_pos, cst, prog->npar + 1, 0,
                                  start_bound_coeff);
  pluto_constraints_selctive_copy(cst_pos, cst, stmt_cst_width, prog->npar + 1,
                                  start_stmt);
  pluto_constraints_selctive_copy(cst_pos, cst, arr_cst_width,
                                  prog->npar + 1 + prog->nvar + prog->npar + 1,
                                  start_arr);
