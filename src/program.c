...
osl_relation_p pluto_matrix_to_osl_access_relation(PlutoMatrix *pmat) {
  ...
  // osl_int_set_si(rl->precision, &rl->m[0][rl->nb_columns-1],
  // pmat->val[0][0]);
  ...
  if (options->data_dist) {
    ...
    for (i = 0; i < narrays; ++i) {
      ...
      arr->tiled_hyperplane = (struct TiledHyperplane *)malloc(
          arr->dim_orig * sizeof(struct TiledHyperplane));
...
PlutoConstraints *pluto_get_new_arr_domain(const Array *arr,
  ...
  pluto_constraints_project_out(newdom, arr->trans_orig->nrows + copy_level,
                                arr->dim_orig);
...
PlutoConstraints *pluto_get_new_domain(const Stmt *stmt) {
  ...
  if (stmt->domain->names) {
    ...
    pluto_constraints_set_names_range(newdom, iterators, 0, 0,
                                      stmt->trans->nrows);
    /* Set parameter names */
...
PlutoAccess **pluto_get_all_distinct_write_vars(const PlutoProg *prog,
                                                int *num) {
  int i, j;
  ...
  *num = 0;
  ...
  for (i = 0; i < nstmts; i++) {
    ...
    for (j = 0; j < stmts[i]->nreads; j++) {
      if (is_access_scalar(stmts[i]->reads[j]))
        continue;
