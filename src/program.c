...
void read_array_bounds_from_file(Array **arr, int narrays){

    FILE *fp = NULL;

    fp = fopen ("array_bounds", "rt");
    ...
    while(1){
        ...
        int dim = 0, npar = 1,  nrows = 0, ncols = 0;
        fscanf(fp, "dim = %d\n", &dim);
        fscanf(fp, "npar = %d\n", &npar);
        fscanf(fp, "nrows = %d\n", &nrows);
        fscanf(fp, "ncols = %d\n", &ncols);
		...
		print_polylib_visual_sets("dim", cst);

		//Update the array bounding constraints
		for(i=0;i<narrays;i++){
			if(!strcmp(arr[i]->text, name)){
				arr[i]->array_bounds = cst;
				break;
			}
		}
	}
...
static __isl_give isl_map *pluto_matrix_schedule_to_isl_map(
        PlutoMatrix *schedule, __isl_take isl_space *dim)
{
    int n_row, n_col;
    isl_ctx *ctx;
    isl_mat *eq, *ineq;
...
PlutoProg *scop_to_pluto_prog(osl_scop_p scop, PlutoOptions *options)
{
    ...
    if(options->data_dist){
    	int narrays = 0;
		PlutoAccess **all_accs = pluto_get_all_accs(prog, &narrays);
		prog->narrays = narrays;
...
//			arr->id = i;
            ...
            arr->parmetric_domain = NULL;
			arr->text = strdup(all_accs[i]->name);

			arr->data_type = strdup("double");

			arr->dim = arr->dim_orig = all_accs[i]->mat->nrows;
...
//				if(symbol->data_type)
//					arr->data_type = strdup(symbol->data_type);
//				arr->dim = arr->dim_orig = symbol->num_of_dimensions;
//				if(arr->dim > prog->max_array_dim)
//					prog->max_array_dim = arr->dim;
//
//				arr->iterators = (char **)malloc(arr->dim * sizeof(char *));
...
////					arr->array_bounds = scoplib_matrix_to_pluto_constraints(symbol->dimensions_bounds);
//
//
//				prog->arrays[i] = arr;
//				i++;
...
PlutoConstraints *pluto_get_new_arr_domain(const Array *arr, PlutoConstraints *domain,
        int copy_level)
{
    int i;
...
int get_const_bound_difference(const PlutoConstraints *cnst, int depth)
{
    ...
    _lcm = 1;
    for (r=0; r<cst->nrows; r++) {
        if (cst->val[r][depth] != 0) _lcm = lcm(_lcm, llabs(cst->val[r][depth]));
    }
    for (r=0; r<cst->nrows; r++) {
        ...
        const PlutoMatrix *acc, int **divs) 
{
    ...
    *divs = malloc(sizeof(int)*acc->nrows);
    ...
    // IF_DEBUG2(printf("Old access function is \n"));
    // IF_DEBUG2(pluto_matrix_print(stdout, acc));;

    // IF_DEBUG2(printf("Stmt trans\n"));
...
/* Get all read write accesses in the program */
PlutoAccess **pluto_get_all_accs(PlutoProg *prog, int *num)
{
    ...
    *num = 0;
    ...
    for (i=0; i<prog->nstmts; i++) {
        if(prog->stmts[i]->nwrites  <1) continue;
        assert(prog->stmts[i]->nwrites == 1);
		if(!is_access_scalar(prog->stmts[i]->writes[0]))
			add_if_new_var(&accs, num, prog->stmts[i]->writes[0]);

        for (j = 0; j < prog->stmts[i]->nreads; ++j) {
        	if(!is_access_scalar(prog->stmts[i]->reads[j]))
				add_if_new_var(&accs, num, prog->stmts[i]->reads[j]);
		}
    }
    return accs;
}
    ...
    *num = 0;
    ...
    for (i=0; i<nstmts; i++) {
        ...
        for (j=0; j<stmts[i]->nreads; j++) {
        	if(is_access_scalar(stmts[i]->reads[j])) continue;
...
/* Get all write accesses in the statements */
PlutoAccess **pluto_get_waccs(Stmt **stmts, int nstmts, int *num)
{
    ...
    *num = 0;
    ...
    for (i=0; i<nstmts; i++) {
        ...
        add_if_new_var(&accs, num, stmts[i]->writes[0]);
    }
    return accs;
}
 ...
 */
static void compute_deps_pet(struct pet_scop *pscop, isl_map **stmt_wise_schedules, 
        PlutoProg *prog, PlutoOptions *options)
{
    int i;
    isl_union_map *empty;
    ...
    for (i=0; i<prog->nstmts; i++) {
        ...
        isl_union_map *s_umap = isl_union_map_intersect_domain(isl_union_map_copy(schedules),
                isl_union_set_from_set(isl_set_copy(pstmt->domain)));

        isl_map *s_map = isl_map_from_union_map(s_umap);

        isl_union_map *lreads =	pet_stmt_collect_accesses(pstmt, pet_expr_access_may_read, 
                0, isl_space_copy(space));
        isl_union_map *lwrites = pet_stmt_collect_accesses(pstmt, pet_expr_access_may_write,  
                0, isl_space_copy(space));
    ...
    if (options->lastwriter && options->distmem) {
        ...
        prog->ntransdeps += extract_deps(prog->transdeps, prog->ntransdeps, prog->stmts, dep_raw, OSL_DEPENDENCE_RAW);
        prog->ntransdeps += extract_deps(prog->transdeps, prog->ntransdeps, prog->stmts, trans_dep_war, OSL_DEPENDENCE_WAR);
        prog->ntransdeps += extract_deps(prog->transdeps, prog->ntransdeps, prog->stmts, dep_waw, OSL_DEPENDENCE_WAW);
    }else{
        prog->transdeps = NULL;
        prog->ntransdeps = 0;
    }
...
static void remove_trivial_dead_code(struct pet_scop *pscop, int *dead) {
    ...
    for (unsigned s = 0; s < pscop->n_stmt; s++)  {
        ...
        dead[s] = 1;
        isl_space *space = isl_set_get_space(pscop->context);
        isl_union_map *writes = pet_stmt_collect_accesses(pstmt,
                pet_expr_access_killed, 0, isl_space_copy(space));

        if (isl_union_map_n_map(writes) != 1) continue;
        ...
        for (unsigned j = 0; j < pscop->n_stmt; j++)  {
            ...
            isl_space *space = isl_set_get_space(pscop->context);
            isl_union_map *writes = pet_stmt_collect_accesses(opstmt,
                    pet_expr_access_may_write, 0, space);
...
static Stmt **pet_to_pluto_stmts(struct pet_scop *pscop, isl_map **stmt_wise_schedules,
        int *nstmts)
{
    int i, j, s, t;
    Stmt **stmts;
    ...
    *nstmts = 0;
    ...
    for(s = 0, t = 0; s < pscop->n_stmt; s++)  {
        ...
        isl_map *s_map = isl_map_from_union_map(
        ...
        PlutoMatrix *trans = isl_map_to_pluto_func(s_map,
                isl_set_dim(pstmt->domain, isl_dim_set), npar);

        isl_map_free(s_map);

        stmts[t] = pluto_stmt_alloc(isl_set_dim(pstmt->domain, isl_dim_set), 
                domain, trans);
        pluto_constraints_free(domain);
        pluto_matrix_free(trans);
        ...
        isl_union_map *reads =	pet_stmt_collect_accesses(pstmt, 
                pet_expr_access_may_read, 0, isl_space_copy(space));
        isl_union_map *writes = pet_stmt_collect_accesses(pstmt,
                pet_expr_access_may_write, 0, space);
...
static __isl_give isl_id_list *generate_names(isl_ctx *ctx, struct pet_stmt *stmt)
{
    ...
    for (i=0; i<isl_set_dim(stmt->domain, isl_dim_set); i++) {
        id = isl_id_alloc(ctx, isl_set_get_dim_name(stmt->domain, isl_dim_set, i), NULL);
	...
	__isl_take isl_printer *p)
{
	...
	isl_schedule *sched = isl_schedule_intersect_domain(isl_schedule_copy(scop->schedule), domain_set);
...
PlutoProg *pet_to_pluto_prog(struct pet_scop *pscop, isl_ctx *ctx, PlutoOptions *options)
{
    ...
    for (i=0; i< pscop->n_stmt; i++) {
        ...
        isl_union_map *stmt_sched_umap =
            isl_union_map_intersect_domain(sched_map,
                    isl_union_set_from_set(isl_set_copy(pstmt->domain)));
        ...
        PlutoProg *prog, PlutoMatrix *access_matrix)
{
    ...
    dim = isl_space_alloc(ctx,prog->npar ,domain_dim,
            2*domain_dim +1);
    dim = set_names(dim, isl_dim_param, prog->params);
    dim = set_names(dim, isl_dim_in,iter);
    dim = isl_space_set_tuple_name(dim, isl_dim_in, name);
...
Array *pluto_get_corrs_array(char *arr_name, const PlutoProg* prog)
{
    ...
    for(k = 0; k<prog->narrays; k++){
        if(!strcmp(prog->arrays[k]->text, arr_name )){
