...
PlutoProg *osl_scop_to_pluto_prog(osl_scop_p scop, PlutoContext *context) {
  ...
  // osl_scop_to_pluto_prog crashes if scop->statement is null, which can happen
  // on empty scop, to fix that, we prevent the call to osl_statement_get_nb_iterators
  prog->nvar = scop_stmt == NULL ? 0 : osl_statement_get_nb_iterators(scop_stmt);
  max_sched_rows = 0;
