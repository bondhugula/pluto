...
#define __DECLARATION_OF_POINTER_TO_new_point(new_point) double (*new_point)[N+2][N+2]
...
void init_array(double point[N+2][N+2][N+2], double new_point[N+2][N+2][N+2]) {
  const int BASE = 1024;
  int i, j, k;
...
void kernel(double point[N+2][N+2][N+2], double new_point[N+2][N+2][N+2]) {
  long int t, i, j, k;
...
#pragma scop
  for (t = 0; t < T; t++) {
    for (i = 1; i < N+1; i++) {
      for (j = 1; j < N+1; j++) {
        for (k = 1; k < N+1; k++) {
          new_point[i][j][k] =   0.125 * (point[i+1][j][k] - 2.0 * point[i][j][k] + point[i-1][j][k])
                                 + 0.125 * (point[i][j+1][k] - 2.0 * point[i][j][k] + point[i][j-1][k])
                                 + 0.125 * (point[i][j][k-1] - 2.0 * point[i][j][k] + point[i][j][k+1])
                                 + point[i][j][k];
        }
      }
    }
...
/* Define our arrays */
  ...
  point = (double(*)[N+2][N+2][N+2]) malloc((N+2)*(N+2)*(N+2)*sizeof(double));
  double (*new_point)[N+2][N+2][N+2];
  new_point = (double(*)[N+2][N+2][N+2]) malloc((N+2)*(N+2)*(N+2)*sizeof(double));
