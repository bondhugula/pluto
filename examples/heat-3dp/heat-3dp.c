...
#pragma ivdep
                for (k = 0; k <= _N; k++) {
                    A[(t+1)%2][i][j][k] =   0.125 * (A[t%2][i==_N?0:i+1][j][k] - 2.0 * A[t%2][i][j][k] + A[t%2][(i==0)?(_N):(i-1)][j][k]) + 0.125 * (A[t%2][i][j==_N?0:j+1][k] - 2.0 * A[t%2][i][j][k] + A[t%2][i][(j==0)?(_N):(j-1)][k]) + 0.125 * (A[t%2][i][j][(k==0)?(_N):(k-1)] - 2.0 * A[t%2][i][j][k] + A[t%2][i][j][k==_N?0:k+1]) + A[t%2][i][j][k];
