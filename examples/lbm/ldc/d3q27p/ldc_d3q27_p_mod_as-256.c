...
#ifndef nX
#define nX 256  // Size of domain along x-axis
#endif
...
#ifndef nZ
#define nZ 256  // Size of domain along z-axis
#endif
...
#ifndef nTimesteps
#define nTimesteps 300  // Number of timesteps for the simulation
#endif
...
int main(void) {
  ...
  for (i=0;i<2;i++){
	  ...
	  for (z=0;z<nZ;z++){
		  grid[i][z]=(double***)malloc(nY*sizeof(double**));
		  for (y=0;y<nY;y++){
			  grid[i][z][y]=(double**)malloc(nX*sizeof(double*));
			  for (x=0;x<nX;x++){
				  grid[i][z][y][x]=(double*)malloc(nK*sizeof(double));
			  }
		  }
	  }
  }
   ...
   * accordingly */
#pragma omp parallel for private (y,x,k)
  for (z = 0; z < nZ; z++) {
    for (y = 0; y < nY; y++) {
      for (x = 0; x < nX; x++) {
        grid[0][z][y][x][0] = w1;
...
#pragma ivdep
        for (x = 0; x < 2*_nX; x++) {
          lbm_kernel(
              ...
              grid[t % 2][z + 0][y + 0][x==0?2*_nX-1:x-1][E],
              grid[t % 2][z + 0][y + 0][x==2*_nX-1?0:x+1][W],
              grid[t % 2][z + 0][y==0?2*_nY-1:y-1][x + 0][N],
              grid[t % 2][z + 0][y==2*_nY-1?0:y+1][x + 0][S],
              grid[t % 2][z==0?2*_nZ-1:z-1][y + 0][x + 0][T],
              grid[t % 2][z==2*_nZ-1?0:z+1][y + 0][x + 0][B],
              grid[t % 2][z + 0][y==0?2*_nY-1:y-1][x==0?2*_nX-1:x-1][NE],
              grid[t % 2][z + 0][y==0?2*_nY-1:y-1][x==2*_nX-1?0:x+1][NW],
              grid[t % 2][z + 0][y==2*_nY-1?0:y+1][x==0?2*_nX-1:x-1][SE],
              grid[t % 2][z + 0][y==2*_nY-1?0:y+1][x==2*_nX-1?0:x+1][SW],
              grid[t % 2][z==0?2*_nZ-1:z-1][y + 0][x==0?2*_nX-1:x-1][TE],
              grid[t % 2][z==0?2*_nZ-1:z-1][y + 0][x==2*_nX-1?0:x+1][TW],
              grid[t % 2][z==2*_nZ-1?0:z+1][y + 0][x==0?2*_nX-1:x-1][BE],
              grid[t % 2][z==2*_nZ-1?0:z+1][y + 0][x==2*_nX-1?0:x+1][BW],
              grid[t % 2][z==0?2*_nZ-1:z-1][y==0?2*_nY-1:y-1][x + 0][TN],
              grid[t % 2][z==0?2*_nZ-1:z-1][y==2*_nY-1?0:y+1][x + 0][TS],
              grid[t % 2][z==2*_nZ-1?0:z+1][y==0?2*_nY-1:y-1][x + 0][BN],
              grid[t % 2][z==2*_nZ-1?0:z+1][y==2*_nY-1?0:y+1][x + 0][BS],
              grid[t % 2][z==0?2*_nZ-1:z-1][y==0?2*_nY-1:y-1][x==0?2*_nX-1:x-1][TNE],
              grid[t % 2][z==0?2*_nZ-1:z-1][y==0?2*_nY-1:y-1][x==2*_nX-1?0:x+1][TNW],
              grid[t % 2][z==0?2*_nZ-1:z-1][y==2*_nY-1?0:y+1][x==0?2*_nX-1:x-1][TSE],
              grid[t % 2][z==0?2*_nZ-1:z-1][y==2*_nY-1?0:y+1][x==2*_nX-1?0:x+1][TSW],
              grid[t % 2][z==2*_nZ-1?0:z+1][y==0?2*_nY-1:y-1][x==0?2*_nX-1:x-1][BNE],
              grid[t % 2][z==2*_nZ-1?0:z+1][y==0?2*_nY-1:y-1][x==2*_nX-1?0:x+1][BNW],
              grid[t % 2][z==2*_nZ-1?0:z+1][y==2*_nY-1?0:y+1][x==0?2*_nX-1:x-1][BSE],
              grid[t % 2][z==2*_nZ-1?0:z+1][y==2*_nY-1?0:y+1][x==2*_nX-1?0:x+1][BSW],
              &grid[(t + 1) % 2][z][y][x][C],
              &grid[(t + 1) % 2][z][y][x][E],
              &grid[(t + 1) % 2][z][y][x][W],
              &grid[(t + 1) % 2][z][y][x][N],
