...
/* Start of CLooG code */
if ((N >= 1) && (T >= 1)) {
  for (t1=-1;t1<=floord(T-1,256);t1++) {
    lbp=max(ceild(t1,2),ceild(512*t1-T+2,512));
    ubp=min(floord(T+N-1,512),floord(256*t1+N+255,512));
#pragma omp parallel for private(lbv,ubv)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(max(0,256*t1),512*t2-N),512*t1-512*t2+1);t3<=min(min(min(T-1,256*t1+511),512*t2+510),512*t1-512*t2+N+511);t3++) {
if(t3%2==0){
        for (t4=max(max(512*t2,t3+1),-512*t1+512*t2+2*t3-511);t4<=min(min(512*t2+511,t3+N),-512*t1+512*t2+2*t3);t4++) {
